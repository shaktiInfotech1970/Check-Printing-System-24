...
namespace CPS.Business
{
    ...
    public class RequestBase : BaseEntity
    {
        ...
        [Display(Name = "telr", Order = 22)]
        [Parse(22)]
        public string telr { get; set; }

        [Display(Name = "telo", Order = 23)]
        [Parse(23)]
        ...
        [Display(Name = "additional_f10", Order = 41)]
        [Parse(41)]
        public string additional_f10 { get; set; }
    }
    ...
    public class RequestDTO : RequestBase
    {
        ...
        public override bool IsValid(IList<ValidationResult> results)
        {
            ...
            try
            {
                ...
                {
                    var repository = new PersistenceBase<RequestDTO>(context);
                    if (repository.FilterBy(w => this.BearerOrder.ToLower() == "bearer" &&  w.Id != this.Id && w.BranchId == this.BranchId && w.AccountNoFull == this.AccountNoFull && (((w.ChequeFrom >= this.ChequeFrom) && (w.ChequeFrom <= this.ChequeTo)) || ((this.ChequeFrom >= w.ChequeFrom) && (this.ChequeFrom <= w.ChequeTo)))).FirstOrDefault() != null)
                    {
                        results.Add(new ValidationResult("Cheque series are already exist."));
                    }
                    if (repository.FilterBy(w => this.BearerOrder.ToLower() == "bearer" && w.Id != this.Id && w.BranchId == this.BranchId && w.AccountNoFull == this.AccountNoFull && w.TransactionCode != this.TransactionCode).FirstOrDefault() != null)
                    {
                        results.Add(new ValidationResult("A/C transaction code is not valid."));
                    }
                    if(this.BearerOrder.ToLower() == "order" && this.SerialNo >=0)
                    {
                        results.Add(new ValidationResult("Serial number for PayOrder must be less than zero."));
                    }
                    if (repository.FilterBy(w => this.BearerOrder.ToLower() == "bearer" && w.Id != this.Id && w.BranchId == this.BranchId && w.SerialNo == this.SerialNo).FirstOrDefault() != null)
                    {
