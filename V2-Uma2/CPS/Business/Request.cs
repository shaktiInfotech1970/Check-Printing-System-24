...
namespace CPS.Business
{
    ...
    public class RequestDTO : RequestBase
    {
        ...
        public override bool IsValid(IList<ValidationResult> results)
        {
            ...
            if (this.TransactionCode != 12) // Not Pay Order
            {
                if (string.IsNullOrWhiteSpace(this.AccountNo))
                {
                    results.Add(new ValidationResult("Account No is compulsory for Savings/Current/CC"));
                }

                if (string.IsNullOrWhiteSpace(this.Name))
                {
                    results.Add(new ValidationResult("Account Name is compulsory for Savings/Current/CC"));
                }
            }
            try
            {
