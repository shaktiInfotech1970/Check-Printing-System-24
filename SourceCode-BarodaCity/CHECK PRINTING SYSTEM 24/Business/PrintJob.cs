...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        private static string GetBranchName(BranchMasterDTO branch)
        {
            var sb = new StringBuilder();
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.Name) && request.Name.Length > 35)
            {
                name = string.Format("{0}...", request.Name.Substring(0, 35));
            }
            if (!string.IsNullOrWhiteSpace(name))
                e.Graphics.DrawString(name, fontBold, brush, x, y);
        ...
        private static string GetStamp(RequestDTO request)
        {
            ...
            //If transaction code is 11(current) or 13(ccod) then do not print joint name or signatories. Print only autorized signatory
            if (request.TransactionCode == 11 || request.TransactionCode == 13)
            {
        ...
        {
            return string.Format("C{0}C {1}A {2}C {3}", (request.ChequeFrom + (currentChequeNo - 1)).ToString("000000"), request.MICRCode, Convert.ToInt32(request.AccountNo).ToString("000000"), request.TransactionCode);
        }
                ...
                foreach (var batch in printerJob.PrintBatches)
                {
                    ...
                    if ((printType == PrintType.ChequeBook && printerPreference.RequestTray != printerPreference.ChequeTray) || printType == PrintType.RePrintRequest)
                    {
                        using (var requestPrintDocument = GetPrintDocument(printerPreference.Name, printerPreference.RequestTray, new PrintPageEventHandler(requestPrintDocument_PrintPage)))
                        {
                            ...
                            if (printType == PrintType.ChequeBook && printerPreference.RequestTray != printerPreference.ChequeTray)
                            {
        ...
        private static void PrintRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            var printAccountNo = section.PrintRequest.Request.AccountNoFullPrint;
            if (section.PrintRequest.Request.TransactionCode == 12)
            {
                printAccountNo = section.PrintRequest.Request.AccountNoFullPrintPayOrder;
            }

            if (BatchInPrinting.RequestLayout.branchAddress1Visble)
                e.Graphics.DrawString(GetBranchName(section.PrintRequest.Branch), font9, brush, x + GetDPI(BatchInPrinting.RequestLayout.branchAddress1X), y + GetDPI(BatchInPrinting.RequestLayout.branchAddress1Y));

            if (BatchInPrinting.RequestLayout.branchAddress2Visble)
                e.Graphics.DrawString(GetBranchName(section.PrintRequest.Branch), font9, brush, x + GetDPI(BatchInPrinting.RequestLayout.branchAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.branchAddress2Y));
            ...
            if (BatchInPrinting.RequestLayout.accountNo1Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, printAccountNo), font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo1X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo1Y));

            if (BatchInPrinting.RequestLayout.accountNo2Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, printAccountNo), font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo2X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo2Y));

            var barcode = GetBarcode(printAccountNo, 15, System.Drawing.RotateFlipType.RotateNoneFlipNone);
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.ChequeLayout.accountNoVisble)
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, printAccountNo), font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY));
