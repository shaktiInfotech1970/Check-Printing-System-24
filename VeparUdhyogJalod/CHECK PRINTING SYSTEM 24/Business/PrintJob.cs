...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        {
            var oLoggedInUser = (UserMasterDTO)App.Current.Windows[0].Tag;
            return string.Format("{0, -10} {1, -5} {2, -5} {3, -20}", oLoggedInUser.UserId, request.PrintJobNo, 1, DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss"));
        }

        private static string GetAuditText(RequestDTO request, int pageNo)
        {
            var oLoggedInUser = (UserMasterDTO)App.Current.Windows[0].Tag;
            return string.Format("{0, -10} {1, -5} {2, -5} {3, -20}", oLoggedInUser.UserId, request.PrintJobNo, pageNo, DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss"));
        }
