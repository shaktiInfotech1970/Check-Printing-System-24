...
namespace CPS.Views.Reports
{
    ...
    public partial class DaywiseChequePrint : UserControl
    {
        ...
        {
            var data = dgDaywiseChequePrint.ItemsSource
                .Cast<dynamic>()
                .GroupBy(x => x.Branch.Id)
                .ToList();

            foreach (var branchGroup in data)
            {
                var branchName = branchGroup.First().Branch.Value;

                Paragraph title = new Paragraph(
                    $"Daywise Cheque Print - Branch: {branchName}\nTransaction Date: {(dtTransactionDate.SelectedDate ?? DateTime.Now.Date):dd MMM yyyy}",
                    new Font(Font.FontFamily.HELVETICA, 15)
                );
                title.Alignment = Element.ALIGN_CENTER;

                dgDaywiseChequePrint.ItemsSource = branchGroup.ToList();

                ReportPDF report = new ReportPDF(
                    dgDaywiseChequePrint,
                    new float[] { 40, 90, 180, 50, 30, 55, 50, 30, 110, 50, 95 }
                );
