    ...
    public class PrintJob
    {
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y, float width, float height)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.City))
                sb.Append(Environment.NewLine + request.City);

            if (!string.IsNullOrWhiteSpace(request.PostalCode))
                sb.Append(", " + request.PostalCode);
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.City))
            {
                e.Graphics.DrawString(request.City, font, brush, x, y);
            }

            if (!string.IsNullOrWhiteSpace(request.PostalCode))
            {
                var prefix = string.IsNullOrWhiteSpace(request.City) ? "" : ", ";
                e.Graphics.DrawString(prefix + request.PostalCode, font, brush, x, y);
            }
        ...
        {
            string last6 = print.Request.AccountNo
                .Substring(Math.Max(0, print.Request.AccountNo.Length - 6));

            int accNo = 0;
            int.TryParse(last6, out accNo);

            return string.Format(
                "C{0}C {1}A {2}C {3}",
                (print.Request.ChequeFrom + (currentChequeNo - 1)).ToString("000000"),
                print.Branch.MICR,
                accNo.ToString("000000"),
        ...
        private static void PrintRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            // Branch Name below Branch Address 1
            e.Graphics.DrawString(
                section.PrintRequest.Branch.Name,
                font9,
                brush,
                x + GetDPI(BatchInPrinting.RequestLayout.branchAddress1X + 1.5f),
                y + GetDPI(BatchInPrinting.RequestLayout.branchAddress1Y + 1.2f)
            );
            ...
            e.Graphics.DrawString(
                ...
                font9,
                brush,
                x + GetDPI(BatchInPrinting.RequestLayout.branchAddress1X + 15.2f),
                y + GetDPI(BatchInPrinting.RequestLayout.branchAddress1Y + 1.2f)
            );
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            if (section.PrintRequest.Request.BearerOrder.Trim().ToLower() == "order"
            && BatchInPrinting.ChequeLayout.accountPayeeVisble)
            {
                var imgAccountPayee = GetAccountPayeeImage();

                // 🔽 Move slightly down
                float posX = x + GetDPI(BatchInPrinting.ChequeLayout.accountPayeeX);
                float posY = y + GetDPI(BatchInPrinting.ChequeLayout.accountPayeeY + 0.2f); // ⬅ move down

                // 🔼 Increase size a bit more
                float imgWidth = GetDPI(1.9f);
                float imgHeight = GetDPI(0.7f);

                // 🎯 Center transform
                e.Graphics.TranslateTransform(posX + imgWidth / 2, posY + imgHeight / 2);
            ...
            int accNo = 0;
            int.TryParse(section.PrintRequest.Request.AccountNo, out accNo);
