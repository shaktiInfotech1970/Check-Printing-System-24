    ...
    public class ImportRequest
    {
        ...
        private bool Parse(string line, RequestDTO importData)
        {
            ...
            foreach (var property in properties)
            {
                ...
                string rawValue = token.Length > order ? token[order].Trim().Trim('"') : string.Empty;
                ...
                if (string.IsNullOrWhiteSpace(rawValue))
                {
                    ...
                    safeValue = property.PropertyType.IsValueType ? Activator.CreateInstance(property.PropertyType) : null;
                }
                else
                {
                    try
                    {
                        ...
                        safeValue = Convert.ChangeType(rawValue, property.PropertyType);
                    }
                    catch
                    {
                        // TryParse fallback for numerics
                        if (property.PropertyType == typeof(int))
                        {
                            int.TryParse(rawValue, out var v); safeValue = v;
                        }
                        else if (property.PropertyType == typeof(long))
                        {
                            long.TryParse(rawValue, out var v); safeValue = v;
                        }
                        else if (property.PropertyType == typeof(decimal))
                        {
                            decimal.TryParse(rawValue, out var v); safeValue = v;
                        }
                        else if (property.PropertyType == typeof(double))
                        {
                            double.TryParse(rawValue, out var v); safeValue = v;
                        }
                        else
                        {
                            // Last fallback = assign raw string
                            safeValue = rawValue;
                        }
                    }
                }
