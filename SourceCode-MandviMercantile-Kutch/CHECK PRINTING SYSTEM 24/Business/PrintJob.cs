    ...
    public class PrintJob
    {
        ...
        private static string GetBranchAddress(BranchMasterDTO branch)
        {
            ...
            if (!string.IsNullOrWhiteSpace(branch.Name))
                sb.Append(string.Format("{0}: {1}", branch.Name, branch.AddressLine1));

            if (!string.IsNullOrWhiteSpace(branch.City))
                sb.Append(string.Format("{0} - {1} DIST: KACHCHH               PAYABLE THRU CTS CLEARING CENTERS", System.Environment.NewLine + branch.City, branch.PostalCode));
        ...
        private static string GetBranchAddress1(BranchMasterDTO branch)
        {
            ...
            if (!string.IsNullOrWhiteSpace(branch.Name))
                sb.Append("THE MANDVI MERCHANTILE CO-OP BANK LTD.");
        ...
        private static void PrintRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            e.Graphics.DrawString("Regu. Busi.Hrs.: Mon to Sat: 9:30 am to 4:30 pm", font6, brush, x + GetDPI(0.5f), y + GetDPI(8.9f));
            e.Graphics.DrawString("Half Day Busi.Hrs. 9:30 am to 1:00 pm (2nd and 4th Sat Closed)", font6, brush, x + GetDPI(0.5f), y + GetDPI(9.3f));
            ...
            e.Graphics.DrawString(string.Format("Tel No: {0}", section.PrintRequest.Request.telr), font7, brush, x + GetDPI(12.4f), y + GetDPI(8.8f));
            e.Graphics.DrawString(string.Format("Mobile No: {0}", section.PrintRequest.Request.mob), font7, brush, x + GetDPI(12.4f), y + GetDPI(9.3f));
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.ChequeLayout.branchAddressVisble)
                e.Graphics.DrawString(GetBranchAddress(section.PrintRequest.Branch), font6, brush, x + GetDPI(BatchInPrinting.ChequeLayout.branchAddressX), y + GetDPI(BatchInPrinting.ChequeLayout.branchAddressY));

            if (BatchInPrinting.ChequeLayout.ifscVisble)
                e.Graphics.DrawString(string.Format("IFS CODE: {0} (BANK NAME: AXIS BANK LTD.)", section.PrintRequest.Branch.IFSC.ToUpper()), font7Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.ifscX), y + GetDPI(BatchInPrinting.ChequeLayout.ifscY));
