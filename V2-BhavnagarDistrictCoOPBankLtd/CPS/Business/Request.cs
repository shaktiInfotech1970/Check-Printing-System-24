...
namespace CPS.Business
{
    ...
    public class RequestBase : BaseEntity
    {
        ...
        private string _accountNoFull; // field
        [Display(Name = "A/C BH", Order = 7)]
        [Required]
        [Parse(7)]
        public string AccountNoFull
        {
            get { return _accountNoFull; }
            set { _accountNoFull = value?.TrimStart('0'); }
        }
    ...
    public class RequestDTO : RequestBase
    {
        ...
        public static List<LookupItem<int, string>> GetBranchOfUnPrintedRequest()
        {
            using (var context = new CPSDbContext())
            {
                ...
                var query = (from r in requestRepository.FilterBy(w => w.IsPrinted == false)
                             ...
                             select new LookupItem<int, string> { Key = b.Id, Value = b.Name });
