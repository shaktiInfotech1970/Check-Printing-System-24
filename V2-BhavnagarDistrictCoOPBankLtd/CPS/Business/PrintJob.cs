    ...
    public class PrintJob
    {
        ...
        private static string GetBranchAddress(BranchMasterDTO branch)
        {
            ...
            if (!string.IsNullOrWhiteSpace(branch.Name))
                sb.Append(string.Format("{0} BR.", branch.Name));
        ...
        {
            // Ensure additional_f9 is padded to 6 digits (leading zeros if needed)
            string sanNoPadded = print.Request.additional_f9.PadLeft(6, '0');
        ...
        private static void PrintRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.RequestLayout.branchAddress2Visble)
                e.Graphics.DrawString(string.Format("{0} BR.", section.PrintRequest.Branch.Name), font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.branchAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.branchAddress2Y));
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            if (BatchInPrinting.ChequeLayout.custom1Visible)
                e.Graphics.DrawString(string.Format("{0}", BatchInPrinting.ChequeLayout.custom1Text), font6, brush, x + GetDPI(BatchInPrinting.ChequeLayout.custom1X), y + GetDPI(BatchInPrinting.ChequeLayout.custom1Y));
