...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        private static string GetStamp(RequestDTO request)
        {
            ...
            {
                sb.Append("For, SULAIMANI CO-OPERATIVE BANK LTD.");
        ...
        {
            //var lastDate = new DateTime(2023, 11, 28);
        ...
        private static void PrintRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.RequestLayout.accountNo1Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", GetAccountTypShotName(section.PrintRequest.AccountType), section.PrintRequest.Request.AccountNoFull), font9, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo1X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo1Y));

            if (BatchInPrinting.RequestLayout.accountNo2Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", GetAccountTypShotName(section.PrintRequest.AccountType), section.PrintRequest.Request.AccountNoFull), font9, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo2X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo2Y));
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            {
                e.Graphics.DrawString("PAY ORDER", font9Bold, brush, x + GetDPI(12f), y + GetDPI(0.2f));
            }
            ...
            if (BatchInPrinting.ChequeLayout.accountNoVisble)
                e.Graphics.DrawString(string.Format("{0}:  {1}", GetAccountTypShotName(section.PrintRequest.AccountType), section.PrintRequest.Request.AccountNoFull), fontBold10, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY));
