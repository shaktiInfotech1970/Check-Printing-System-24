...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        private static string GetStamp(RequestDTO request)
        {
            ...
            if (request.TransactionCode == 12)
            {
                Name = "Pay Order For Value Received";
                request.Signatory1 = "Authorized Signatories";
            }
            else if (request.TransactionCode != 10)
            {
                Name = string.Format("For {0}", Name);
                request.Signatory1 = "Authorized Signatories";
            }
            var sb = new StringBuilder();
        ...
        private static string GetMICR(PrintRequest print, int currentChequeNo)
        {
            ...
            {
                shortAccountNo = Convert.ToInt32(print.Request.AccountNo.Substring(Math.Max(0, print.Request.AccountNo.Length - 6))).ToString("000000");
            }
            return string.Format("C{0}C {1}A {2}C {3}", (print.Request.ChequeFrom + (currentChequeNo - 1)).ToString("000000"), print.Branch.MICR, shortAccountNo, print.Request.TransactionCode);
        }
        ...
        private static void PrintRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            e.Graphics.DrawString(section.PrintRequest.Branch.Name, font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress1X), y + GetDPI(BatchInPrinting.RequestLayout.chequeTo1Y - 1f));
            e.Graphics.DrawString(section.PrintRequest.Branch.Name, font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.chequeTo2Y - 1f));
            ...
            if (BatchInPrinting.RequestLayout.nameAddress2Visble)
                PrintNameAddress(e, section.PrintRequest.Request, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y), 360, 110);

            if (BatchInPrinting.RequestLayout.accountNo1Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.TransactionCode == 12 ? "" : section.PrintRequest.Request.AccountNoFull), font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo1X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo1Y));

            if (BatchInPrinting.RequestLayout.accountNo2Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.TransactionCode == 12 ? "" : section.PrintRequest.Request.AccountNoFull), font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo2X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo2Y));

            var barcode = GetBarcode(section.PrintRequest.Request.AccountNoFull, 15, System.Drawing.RotateFlipType.RotateNoneFlipNone);
            if (BatchInPrinting.RequestLayout.barcode1Visble && section.PrintRequest.Request.TransactionCode != 12)
            ...
            if (BatchInPrinting.RequestLayout.barcode2Visble && section.PrintRequest.Request.TransactionCode != 12)
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            if (BatchInPrinting.ChequeLayout.accountNoVisble && section.PrintRequest.Request.TransactionCode != 12)
            ...
            var imgBarcode = GetBarcode(string.Format("{0}{1}{2}{3}", (section.PrintRequest.Request.ChequeFrom + (section.SequenceNo - 1)).ToString("000000"), section.PrintRequest.Request.MICRCode, section.PrintRequest.Request.TransactionCode == 12 ? "      " : Convert.ToInt32(section.PrintRequest.Request.AccountNo).ToString("000000"), section.PrintRequest.Request.TransactionCode), 12, System.Drawing.RotateFlipType.Rotate270FlipNone);


            if (BatchInPrinting.ChequeLayout.barcodeVisble && section.PrintRequest.Request.TransactionCode != 12)
