...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        private static string GetStamp(RequestDTO request)
        {
            ...
            if (request.TransactionCode == 12)   // Pay Order
            {
                // Same line
        ...
        private static void PrintRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.RequestLayout.accountNo1Visble)
                e.Graphics.DrawString(string.Format("{0} {1}: {2}", section.PrintRequest.Request.additional_f1, section.PrintRequest.AccountType.Name, section.PrintRequest.Request.AccountNoFull), font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo1X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo1Y));

            if (BatchInPrinting.RequestLayout.accountNo2Visble)
                e.Graphics.DrawString(string.Format("{0} {1}: {2}", section.PrintRequest.Request.additional_f1, section.PrintRequest.AccountType.Name, section.PrintRequest.Request.AccountNoFull), font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo2X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo2Y));
            ...
            //Customized as per bank request
            e.Graphics.DrawString(string.Format("{0}-{1}", section.PrintRequest.Request.City, section.PrintRequest.Request.PostalCode), font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo1X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo1Y - 0.7f));
            e.Graphics.DrawString(string.Format("Contact: {0}", section.PrintRequest.Request.telo), font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo1X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo1Y + 1.5f));
            e.Graphics.DrawString(string.Format("Customer ID: {0}", section.PrintRequest.Request.additional_f9), font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo1X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo1Y + 2.5f));
            //Customized as per bank request
        ...
        private static void PrintBlankRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.RequestLayout.accountNo1Visble)
                e.Graphics.DrawString(string.Format("{0} {1}: {2}", section.PrintRequest.Request.additional_f1, section.PrintRequest.AccountType.Name, section.PrintRequest.Request.AccountNoFull), font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo1X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo1Y));
            ...
            if (BatchInPrinting.RequestLayout.accountNo2Visble)
                e.Graphics.DrawString(string.Format("{0} {1}: {2}", section.PrintRequest.Request.additional_f1, section.PrintRequest.AccountType.Name, section.PrintRequest.Request.AccountNoFull), font8Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo2X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo2Y));
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            if (section.PrintRequest.Request.TransactionCode == 12)
            {
                e.Graphics.DrawString(
                    ...
                    font9Bold,
                    brush,
                    x + GetDPI(BatchInPrinting.ChequeLayout.custom1X + 14f),
                    y + GetDPI(BatchInPrinting.ChequeLayout.custom1Y + 1f)
                );
            }
            ...
            {
                e.Graphics.DrawString(
                    section.PrintRequest.Request.additional_f1,  // your value
                    font9Bold,
                    brush,
                    x + GetDPI(BatchInPrinting.ChequeLayout.custom1X + 16f),
                    y + GetDPI(BatchInPrinting.ChequeLayout.custom1Y + 0.5f)
                );
            }

            if (BatchInPrinting.ChequeLayout.custom2Visible
            && section.PrintRequest.Request.TransactionCode == 12)
            {
                e.Graphics.DrawString(
                    "Not Over As ____________________________",
                    font9, // or font9Bold if you prefer
                    brush,
                    x + GetDPI(BatchInPrinting.ChequeLayout.custom2X + 7f),
                    y + GetDPI(BatchInPrinting.ChequeLayout.custom2Y + 8f)
                );
            }
            ...
            if (BatchInPrinting.ChequeLayout.stampVisble)
            {
                ...
                // If Account Type is Pay Order (Id = 12)
                if (section.PrintRequest.AccountType.Id == 12)
                {
                    stampText = "Officer      Manager";
                }
                else
                {
                    stampText = GetStamp(section.PrintRequest.Request);
                }
