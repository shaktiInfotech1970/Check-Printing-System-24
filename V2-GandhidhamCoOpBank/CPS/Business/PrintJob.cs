    ...
    public class PrintJob
    {
        ...
        private static string GetBranchAddress(BranchMasterDTO branch)
        {
            ...
            if (!string.IsNullOrWhiteSpace(branch.City))
                sb.Append(string.Format("{0}{1} {2} - {3}", System.Environment.NewLine, branch.City, "Gujarat", branch.PostalCode));
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.Name))
                e.Graphics.DrawString(request.Name, font7Bold, brush, x, y);

            y += 14;

            if (!string.IsNullOrWhiteSpace(request.Address1))
                e.Graphics.DrawString(request.Address1, font6Bold, brush, x, y);

            y += 13;
            if (!string.IsNullOrWhiteSpace(request.Address2))
                e.Graphics.DrawString(request.Address2, font6Bold, brush, x, y);

            y += 13;
            if (!string.IsNullOrWhiteSpace(request.Address3))
                e.Graphics.DrawString(request.Address3, font6Bold, brush, x, y);

            y += 13;
            if (!string.IsNullOrWhiteSpace(request.City))
                e.Graphics.DrawString(string.Format("{0} - {1}     {2}", request.City, request.PostalCode, string.IsNullOrWhiteSpace(request.mob) ? string.Empty : request.mob), font6Bold, brush, x, y);
        }
        ...
        private static string GetAccountTypeShortName(int transactionCode)
        {
        ...
        {
            var lastDate = new DateTime(2026,01, 10);
        ...
        private static void PrintBlankRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            e.Graphics.DrawString("Regu. Busi. Hrs.: " + section.PrintRequest.Branch.Time1, font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 1.4f));
            e.Graphics.DrawString("Weekly Holiday : " + section.PrintRequest.Branch.Time2, font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 1.8f));
            e.Graphics.DrawString(string.Format("Branch Email-1 : {0}", section.PrintRequest.Branch.Email), font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.2f));
            e.Graphics.DrawString(string.Format("Branch Email-2 : {0}", section.PrintRequest.Branch.Email2), font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.6f));
            e.Graphics.DrawString(string.Format("Web Address : {0}", oBankMasterDTO.WebAddress), font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 3f));
            e.Graphics.DrawString(string.Format("Contact No.: {0}    {1}", section.PrintRequest.Branch.Telephone1, section.PrintRequest.Branch.Telephone2), font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 3.4f));
            ...
            e.Graphics.DrawString("THE MANAGER", font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.branchAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.branchAddress2Y - 0.4f));
            ...
            e.Graphics.DrawString("Please issue _________Cheque books containig_________Leaves", font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.branchAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.branchAddress2Y + 2.3f));
            e.Graphics.DrawString("Cheque Book : Payable At Par", font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.branchAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.branchAddress2Y + 3.2f));
            if (BatchInPrinting.RequestLayout.accountNo2Visble)
            ...
            //e.Graphics.DrawString(string.Format("Tel No: {0}", section.PrintRequest.Request.telr), font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 2.5f));
            e.Graphics.DrawString(string.Format("Mobile No: {0}", section.PrintRequest.Request.mob), font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 2.5f));
            e.Graphics.DrawString("Signature Verified / Authorized", font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X + 9f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 0.8f));
            ...
            RectangleF stampRect = new RectangleF(x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X - 1.8f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 0.8f), 450, 110);
            ...
            /* START Audit Text */
            var auditText = GetAuditText(section.PrintRequest.Request);
            var auditTextSize = e.Graphics.MeasureString(auditText, font6Bold);
            e.Graphics.RotateTransform(-90);

            if (BatchInPrinting.RequestLayout.audiText1Visble)
                e.Graphics.DrawString(auditText, font6Bold, brush, new PointF((-y) + (-auditTextSize.Width) + (-GetDPI(BatchInPrinting.RequestLayout.audiText1Y)), x + GetDPI(BatchInPrinting.RequestLayout.audiText1X)));
            //BatchInPrinting.RequestLayout.audiText1X=0.05f

            if (BatchInPrinting.RequestLayout.audiText2Visble)
                e.Graphics.DrawString(auditText, font6Bold, brush, new PointF((-y) + (-auditTextSize.Width) + (-GetDPI(BatchInPrinting.RequestLayout.audiText2Y)), x + GetDPI(BatchInPrinting.RequestLayout.audiText2X)));
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            if (BatchInPrinting.ChequeLayout.branchAddressVisble)
                e.Graphics.DrawString(GetBranchAddress(section.PrintRequest.Branch), font7Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.branchAddressX), y + GetDPI(BatchInPrinting.ChequeLayout.branchAddressY));

            if (BatchInPrinting.ChequeLayout.ifscVisble)
                e.Graphics.DrawString(string.Format("IFSC: {0}", section.PrintRequest.Branch.IFSC), font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.ifscX), y + GetDPI(BatchInPrinting.ChequeLayout.ifscY));
            ...
            if (BatchInPrinting.ChequeLayout.custom1Visible)
                e.Graphics.DrawString(string.Format("{0}", GetAccountTypeShortName(section.PrintRequest.Request.TransactionCode), section.PrintRequest.Request.AccountNoFull), font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.custom1X), y + GetDPI(BatchInPrinting.ChequeLayout.custom1Y));
            ...
            if (BatchInPrinting.ChequeLayout.audiTextVisble)
            {
                ...
                var auditTextSize = e.Graphics.MeasureString(auditText, font4);
                e.Graphics.RotateTransform(-90);
                e.Graphics.DrawString(auditText, font4, brush, new PointF((-y) + (-auditTextSize.Width) + GetDPI(-BatchInPrinting.ChequeLayout.audiTextY), x + GetDPI(BatchInPrinting.ChequeLayout.audiTextX)));
                e.Graphics.ResetTransform();

                // First Print History                
                if (!string.IsNullOrWhiteSpace(printChequeBookAuditText))
                {
                    var auditHistoryTextSize = e.Graphics.MeasureString(printChequeBookAuditText, font4);
                    e.Graphics.RotateTransform(-90);
                    e.Graphics.DrawString(printChequeBookAuditText, font4, brush, new PointF((-y) + (-auditHistoryTextSize.Width) + GetDPI(-BatchInPrinting.ChequeLayout.audiTextY), ((x + GetDPI(BatchInPrinting.ChequeLayout.audiTextX)) - GetDPI(0.2f))));
                    e.Graphics.ResetTransform();
                }
            }
