...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        private static string GetBranchAddress(PrintRequest request)
        {
            var sb = new StringBuilder();

            if (!string.IsNullOrWhiteSpace(request.Branch.Name))
                sb.Append(request.Branch.Name);

            if (!string.IsNullOrWhiteSpace(request.Branch.AddressLine1))
                sb.Append(": " + request.Branch.AddressLine1);

            if (!string.IsNullOrWhiteSpace(request.Branch.AddressLine2))
                sb.Append(", " + request.Branch.AddressLine2);

            if (!string.IsNullOrWhiteSpace(request.Branch.AddressLine3))
                sb.Append(", " + request.Branch.AddressLine3);

            if (!string.IsNullOrWhiteSpace(request.Branch.City))
                sb.Append(string.Format(", {0} - {1}", request.Branch.City, request.Branch.PostalCode));
        ...
        private static string GetBranchAddress1(BranchMasterDTO branch)
        {
            ...
            if (!string.IsNullOrWhiteSpace(branch.Name))
                sb.Append("THE BUSINESS CO-OP. BANK LTD.");
            ...
            if (!string.IsNullOrWhiteSpace(branch.City))
                sb.Append(string.Format("{0} - {1}", System.Environment.NewLine + branch.City, branch.PostalCode));

            return sb.ToString();
        }

        private static string GetBranchAddress2(BranchMasterDTO branch)
        {
            var sb = new StringBuilder();
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y, float width, float height)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.Address2))
                sb.Append(string.Format(" {0}", request.Address2));

            if (!string.IsNullOrWhiteSpace(request.Address3))
                sb.Append(string.Format(" {0}", request.Address3));

            if (!string.IsNullOrWhiteSpace(request.Address4))
                sb.Append(string.Format(" {0}", request.Address4));

            if (!string.IsNullOrWhiteSpace(request.City))
                sb.Append(string.Format(" {0}", request.City));

            if (!string.IsNullOrWhiteSpace(request.PostalCode))
                sb.Append(string.Format(" - {0}", request.PostalCode));
        ...
        private static string GetStamp(RequestDTO request)
        {
            ...
            {
                sb.Append("For The BUSINESS CO-OP. BANK LTD.");
                sb.Append(Environment.NewLine + Environment.NewLine + Environment.NewLine + Environment.NewLine + Environment.NewLine);
                sb.Append("Code No." + new string(' ', 30) + "Code No.");
                sb.Append(Environment.NewLine);
                sb.Append("Authorized Signatory" + new string(' ', 10) + "Authorized Signatory");
            }
            else
            {
                if (!string.IsNullOrWhiteSpace(request.Name))
        ...
        private static string GetMICR(RequestDTO request, int currentChequeNo)
        {
            if (IsPayOrder(request.TransactionCode, request.BearerOrder))
            {
                return string.Format("C{0}C {1}A {2}  {3}", (request.ChequeFrom + (currentChequeNo - 1)).ToString("000000"), request.MICRCode, new string(' ', 6), request.TransactionCode);                
            }
            else
            {
        ...
        {
            var lastDate = new DateTime(2027, 5, 15);
        ...
        private static void PrintRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            /* START Left Section */
            e.Graphics.DrawString(section.PrintRequest.Branch.ShortName, font7Bold, brush, x + GetDPI(8.0f), y + GetDPI(1f));
            e.Graphics.DrawString(GetBranchAddress1(section.PrintRequest.Branch), font7, brush, x + GetDPI(0.2f), y + GetDPI(0f));
            if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder))
            {
                e.Graphics.DrawString(string.Format("{0}", "Pay Order"), font7Bold, brush, x + GetDPI(1.6f), y + GetDPI(2f));
            }
            else
            {
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.AccountNoFull), font7Bold, brush, x + GetDPI(1.6f), y + GetDPI(2f));
                PrintNameAddress(e, section.PrintRequest.Request, x + GetDPI(1.6f), y + GetDPI(2.7f), 260, 110);
            }
            var barcode = GetBarcode(section.PrintRequest.Request.AccountNoFull, 15, System.Drawing.RotateFlipType.RotateNoneFlipNone);
            e.Graphics.DrawImage(barcode, x + GetDPI(1.9f), y + GetDPI(5.2f));
            ...
            e.Graphics.DrawString("Regu. Busi.Hrs.: 10:00 AM To 06:00 PM", font6, brush, x + GetDPI(0.5f), y + GetDPI(8.9f));
            e.Graphics.DrawString("Weekly Holiday: Sunday", font6, brush, x + GetDPI(0.5f), y + GetDPI(9.3f));
            e.Graphics.DrawString(string.Format("Branch Email - 1: {0}", section.PrintRequest.Branch.Email), font6, brush, x + GetDPI(0.5f), y + GetDPI(9.7f));
            ...
            //e.Graphics.DrawString(string.Format("Contact No. : {0}", section.PrintRequest.Branch.Telephone1), font6, brush, x + GetDPI(0.5f), y + GetDPI(10.5f));
            /* END Left Section */
            ...
            e.Graphics.DrawString(GetBranchAddress2(section.PrintRequest.Branch), font7, brush, x + GetDPI(12.3f), y + GetDPI(1.8f));
            e.Graphics.DrawString("Date : ________________", font7Bold, brush, x + GetDPI(22f), y + GetDPI(1.8f));
            e.Graphics.DrawString(string.Format("Cheque no. From: {0} To {1}", section.PrintRequest.Request.ChequeFrom.ToString("00000"), section.PrintRequest.Request.ChequeTo.ToString("00000")), font7Bold, brush, x + GetDPI(12.4f), y + GetDPI(3.5f));

            e.Graphics.DrawString("Please issue _________Cheque books containig_________Leaves", font7Bold, brush, x + GetDPI(12.4f), y + GetDPI(3.9f));
            ...
            if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder))
            {
                e.Graphics.DrawString(string.Format("{0}", "Pay Order"), font8Bold, brush, x + GetDPI(12.4f), y + GetDPI(6f));
            }
            else
            {
                ...
                PrintNameAddress(e, section.PrintRequest.Request, x + GetDPI(12.4f), y + GetDPI(6.6f), 450, 110);
            }
            var customerContact = string.IsNullOrWhiteSpace(section.PrintRequest.Request.mob) ? section.PrintRequest.Request.telr : section.PrintRequest.Request.mob;
            e.Graphics.DrawString($"Contact No:{customerContact}", font7, brush, x + GetDPI(12.4f), y + GetDPI(8.8f));
            //e.Graphics.DrawString("Mobile No:", font7, brush, x + GetDPI(12.4f), y + GetDPI(9.3f));
            e.Graphics.DrawString("Signature Verified / Authorized", font6, brush, x + GetDPI(22.3f), y + GetDPI(8.5f));
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.ChequeLayout.custom5Visible)
                e.Graphics.DrawString(section.PrintRequest.Branch.ShortName, font6Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.custom5X), y + GetDPI(BatchInPrinting.ChequeLayout.custom5Y));

            if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder))
            {
                RectangleF stampRect = new RectangleF(x, y + GetDPI(0.2f), 550, 10);
                StringFormat stringFormat = new StringFormat();
                stringFormat.Alignment = StringAlignment.Far;
            ...
            if (BatchInPrinting.ChequeLayout.branchAddressVisble)
                e.Graphics.DrawString(GetBranchAddress(section.PrintRequest), font6Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.branchAddressX), y + GetDPI(BatchInPrinting.ChequeLayout.branchAddressY));
            ...
            if (BatchInPrinting.ChequeLayout.accountNoVisble)
            {
                if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder))
                {
                    e.Graphics.DrawString(string.Format("{0}", "On Account of"), font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY));
                }
                else
                {
            ...
            if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder))
            {
                e.Graphics.DrawString("NOT OVER__________________", font9, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX + 5.8f), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY + 1.8f));
            }
