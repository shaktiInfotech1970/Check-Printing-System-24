...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        {
            string chequeNumber = (print.Request.ChequeFrom + (currentChequeNo - 1)).ToString("000000");
            string micrCode = print.Branch.MICR;
            string lastSix = string.Empty;
            if (!string.IsNullOrEmpty(print.Request.AccountNo) && print.Request.AccountNo.Length >= 6)
            {
                string accPart = print.Request.AccountNo.Substring(print.Request.AccountNo.Length - 6);
                // If it's not all zeros, print 6 digits; otherwise, keep it blank (6 spaces)
                if (accPart != "000000")
                    lastSix = Convert.ToInt32(accPart).ToString("000000");
                else
                    lastSix = "      ";
            }
            else
