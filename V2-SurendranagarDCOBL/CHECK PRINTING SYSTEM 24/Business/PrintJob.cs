    ...
    public class PrintJob
    {
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y, float width, float height)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.City))
                sb.Append(Environment.NewLine+string.Format("{0} - {1}", request.City,request.PinCode));
            if (!string.IsNullOrWhiteSpace(request.Address4))
                sb.Append(Environment.NewLine+"Contact No :"+request.Address4);
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y)
        {
            ...
            y += 13;
            if (!string.IsNullOrWhiteSpace(request.City))
                e.Graphics.DrawString(string.Format("{0}, {1}", request.City, request.PinCode), font, brush, x, y);
            y += 13;
            if (!string.IsNullOrWhiteSpace(request.Address4))
                e.Graphics.DrawString(request.Address4, font, brush, x, y);
        ...
        private static string GetStamp(RequestDTO request)
        {
            ...
            if (request.TransactionCode == 10)
            {
                if (!string.IsNullOrWhiteSpace(request.SignatureJointName1))
                    sb.Append(Environment.NewLine + request.SignatureJointName1);

                if (!string.IsNullOrWhiteSpace(request.SignatureJointName2))
                    sb.Append(Environment.NewLine + request.SignatureJointName2);
            }
            else if (request.TransactionCode == 13 || request.TransactionCode == 11)
            {
            ...
            {
                if (!string.IsNullOrWhiteSpace(request.SignatureJointName1) || !string.IsNullOrWhiteSpace(request.SignatureJointName2))
                {
                    sb.Append(Environment.NewLine + Environment.NewLine + Environment.NewLine + Environment.NewLine + Environment.NewLine);
                }
                if (!string.IsNullOrWhiteSpace(request.SignatureJointName1))
                {
                    sb.Append(request.SignatureJointName1);
                }
                if (!string.IsNullOrWhiteSpace(request.SignatureJointName2))
                {
                    sb.Append(Environment.NewLine + request.SignatureJointName2);
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            e.Graphics.DrawString(
                ...
                font9,
                brush,
                x + GetDPI(BatchInPrinting.ChequeLayout.ifscX -1),
                y + GetDPI(BatchInPrinting.ChequeLayout.ifscY + 8)
            );
