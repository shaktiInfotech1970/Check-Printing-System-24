...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y, float width, float height)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.mob))
                sb.Append("Contact No :" + request.mob);
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y)
        {
            ...
            y += 13;
            if (!string.IsNullOrWhiteSpace(request.mob))
                e.Graphics.DrawString(string.Copy("Contact No : ") + request.mob, fontBold, brush, x, y);
        }
       ...
       // }
        ...
        private static void PrintBlankRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.RequestLayout.accountNo2Visble)
                e.Graphics.DrawString("BANKER'S CHEQUE", font8Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo2X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo2Y));
            ...
            e.Graphics.DrawString(string.Format("Contact No. : {0}", section.PrintRequest.Branch.Telephone1), font8Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.6f));
            ...
            e.Graphics.DrawString("Please issue _________Cheque books containig_________Leaves", font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2X), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2Y -0.7f));
            ...
            e.Graphics.DrawString("Signature of Bearer", font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 2.9f));
            e.Graphics.DrawString("Received By", font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 3.7f));
            RectangleF stampRect = new RectangleF(x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X - 1.2f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 0.6f), 450, 110);
           ...
           // if (section.PrintRequest.Request.BearerOrder.ToLower() == "order")
            ...
            if (BatchInPrinting.ChequeLayout.accountNoVisble)
            {
                if (section.PrintRequest.Request.BearerOrder.ToLower() == "bearer")
                    ...
                    e.Graphics.DrawString("BANKER'S CHEQUE", font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY));
            }

           // if (section.PrintRequest.Request.BearerOrder.ToLower() == "order")
             //   e.Graphics.DrawString("Not Over Rs. ________________", font8, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX) + GetDPI(2.0f), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY) + GetDPI(9.7f));
           
            if (IsPayOrder(section.PrintRequest.Request.TransactionCode))
            {
                e.Graphics.DrawString("NOT OVER RS. ______________________", font7, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX + 6f), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY + 1.8f));
            }
            if (IsPayOrder(section.PrintRequest.Request.TransactionCode))
            {
                e.Graphics.DrawString("ON ACCOUNT OF _____________________", font7, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX + 6f), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY + 3f));
            }
