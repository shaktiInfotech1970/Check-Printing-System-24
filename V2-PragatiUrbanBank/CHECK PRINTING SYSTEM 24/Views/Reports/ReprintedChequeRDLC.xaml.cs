...
{
    /// <summary>
    /// Interaction logic for ReprintedChequeRDLC.xaml
    /// </summary>
    public partial class ReprintedChequeRDLC : UserControl
    {
        public ReprintedChequeRDLC()
        {
            InitializeComponent();
            BindComboBox();
        }
        ...
        private void RenderReport()
        {
            ...
            _reportviewer.LocalReport.ReportEmbeddedResource = "CPS.Views.Reports.Layout.ReprintedCheque.rdlc";
            _reportviewer.LocalReport.EnableExternalImages = true;

            var param = new ReportParameter[2];
            param[0] = new ReportParameter("BankName", string.Format("{0}", BankMasterDTO.GetBankName()));
            param[1] = new ReportParameter("Title", string.Format("Re-Printed Cheque:- From:{0}  - To:{1}", (dtPrintDateFrom.SelectedDate ?? DateTime.Now.Date).ToString("dd MMM yyyy"), (dtPrintDateTo.SelectedDate ?? DateTime.Now.Date).ToString("dd MMM yyyy")));
            _reportviewer.LocalReport.SetParameters(param);

            ReportDataSource reportDataSource = new ReportDataSource();
            reportDataSource.Name = "ReprintedChequeReportDataSet";
        ...
        private List<ReprintedChequeReport> GetData(int branchId, int accountType, DateTime transactionfromDate, DateTime transactiontoDate)
        {
            using (var context = new CPSDbContext())
            {
                ...
                var query = (from r in repositoryRequest.GetAll()
                             ...
                             select new ReprintedChequeReport
                             {
                                 SerialNo = r.SerialNo,
                ...
                var response = query.OrderBy(o => o.ChequeNoFrom).ToList();
                if (response.Count == 0)
                {
                    return new List<ReprintedChequeReport>();
                }
                return response;
            }
        }
    }
    public class ReprintedChequeReport
    {
