...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y)
        {
            ...
            var filterContacts = contacts.Where(s => !string.IsNullOrWhiteSpace(s) && Regex.IsMatch(s, @"\d")).ToList();
            if (filterContacts != null && filterContacts.Count > 0)
            {
                y += 13;
                e.Graphics.DrawString(string.Format("{0} {1}", "Contact No:", string.Join(", ", filterContacts)), font, brush, x, y);
            }
        }
        ...
        {
            var lastDate = new DateTime(2072, 10, 09);
