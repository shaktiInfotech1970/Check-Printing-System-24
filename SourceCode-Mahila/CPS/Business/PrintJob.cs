...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        {
            const string fixedMicrCode = "390802002";

            return string.Format(
                "C{0}C {1}A {2}C {3}",
                (request.ChequeFrom + (currentChequeNo - 1)).ToString("000000"),
                fixedMicrCode,
                Convert.ToInt32(request.AccountNo.Substring(Math.Max(0, request.AccountNo.Length - 6))).ToString("000000"),
        ...
        {
            var installationDate = new DateTime(2026, 11, 9);
            if (DateTime.Now.Date > installationDate.Date.AddDays(365))
            {
