    ...
    public class ImportRequest
    {
        ...
        private bool Parse(string line, RequestDTO importData)
        {
            ...
            var properties = importData.GetType().GetProperties()
                .Where(w => w.GetCustomAttributes(typeof(DisplayAttribute), false).Count() > 0)
                .OrderBy(o => (o.GetCustomAttributes(typeof(DisplayAttribute), false)
                .FirstOrDefault() as DisplayAttribute).GetOrder());

            var index = 0;

            foreach (var property in properties)
            {
                var raw = (index < token.Length ? token[index] : "").Trim().Trim('"');
                index++;
                ...
                try
                {
                    ...
                    {
                        int v;
                        value = int.TryParse(raw, out v) ? v : 0;
                    }
                    else if (targetType == typeof(long))
                    {
                        long v;
                        value = long.TryParse(raw, out v) ? v : 0;
                    }
                    else
                    {
                        value = string.IsNullOrWhiteSpace(raw)
                            ? (targetType == typeof(string) ? "" : Activator.CreateInstance(targetType))
                            : Convert.ChangeType(raw, targetType);
                    }

                    property.SetValue(importData, value, null);
                }
                catch
                {
                    property.SetValue(importData,
                        property.PropertyType == typeof(string) ? "" : Activator.CreateInstance(property.PropertyType),
                        null);
                }
            }

            // 🔥 FINAL FIX: correct shifted fields explicitly (only these lines added)

            importData.PostalCode = token.Length > 20 ? token[20].Trim() : "";

            importData.NoOfChequeBook = token.Length > 24 && int.TryParse(token[24], out var b) ? b : 0;
            importData.NoOfCheque = token.Length > 25 && int.TryParse(token[25], out var c) ? c : 0;

            importData.BearerOrder = token.Length > 26 ? token[26].Trim() : "";
            importData.AtPar = token.Length > 27 ? token[27].Trim() : "";

            importData.ChequeFrom = token.Length > 29 && int.TryParse(token[29], out var f) ? f : 0;
            importData.ChequeTo = token.Length > 30 && int.TryParse(token[30], out var t) ? t : 0;
