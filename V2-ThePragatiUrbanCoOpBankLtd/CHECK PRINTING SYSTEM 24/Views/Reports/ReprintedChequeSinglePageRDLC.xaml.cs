...
{
    /// <summary>
    /// Interaction logic for ReprintChequeSinglePageRDLC.xaml
    /// </summary>
    public partial class ReprintedChequeSinglePageRDLC : UserControl
    {
        public ReprintedChequeSinglePageRDLC()
        {
            InitializeComponent();
            BindComboBox();
        }
        ...
        private void RenderReport()
        {
            ...
            _reportviewer.LocalReport.ReportEmbeddedResource = "CPS.Views.Reports.Layout.ReprintedChequeSinglePage.rdlc";
            _reportviewer.LocalReport.EnableExternalImages = true;

            var param = new ReportParameter[2];
            param[0] = new ReportParameter("BankName", string.Format("{0}", BankMasterDTO.GetBankName()));
            param[1] = new ReportParameter("Title", string.Format("Re-Printed Cheque Single Page:- From:{0}  - To:{1}", (dtPrintDateFrom.SelectedDate ?? DateTime.Now.Date).ToString("dd MMM yyyy"), (dtPrintDateTo.SelectedDate ?? DateTime.Now.Date).ToString("dd MMM yyyy")));
            _reportviewer.LocalReport.SetParameters(param);

            ReportDataSource reportDataSource = new ReportDataSource();
            reportDataSource.Name = "ReprintedChequeSinglePageReportDataSet";
        ...
        private List<ReprintedChequeSinglePageReport> GetData(int branchId, int accountType, DateTime transactionfromDate, DateTime transactiontoDate)
        {
            using (var context = new CPSDbContext())
            {
                ...
                var query = (from r in repositoryRequest.GetAll()
                             ...
                             select new ReprintedChequeSinglePageReport
                             {
                                 SerialNo = r.SerialNo,
                                 ...
                                 NoOfCheque = ph.ChequeNoTo - ph.ChequeNoFrom + 1,
                                 CreatedOn = ph.CreatedOn,
                                 CreatedBy = ph.CreatedBy                                 
                             });
                var response = query.Where(o => o.NoOfCheque > 0).OrderBy(o => o.ChequeNoFrom).ToList();                
                if (response.Count == 0)
                {
                    return new List<ReprintedChequeSinglePageReport>();
    ...
    public class ReprintedChequeSinglePageReport
    {
