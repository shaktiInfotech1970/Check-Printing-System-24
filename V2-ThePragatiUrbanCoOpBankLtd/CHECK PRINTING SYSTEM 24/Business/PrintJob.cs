    ...
    public class PrintJob
    {
        ...
        private static string GetBranchAddress(BranchMasterDTO branch)
        {
            ...
            if (!string.IsNullOrWhiteSpace(branch.City))
                sb.Append(string.Format(",{0}{1} - {2}, Dist. Nashik", System.Environment.NewLine, branch.City, branch.PostalCode));
        ...
        private static string GetBranchAddress1(BranchMasterDTO branch, string bankName)
        {
            ...
            if (!string.IsNullOrWhiteSpace(branch.City))
                sb.Append(string.Format("{0}{1} - {2}, {3}", "," + System.Environment.NewLine, branch.City, branch.PostalCode, "Dist. Nashik"));
        ...
        {
            if (IsPayOrder(print.Request.TransactionCode))
                return string.Format("C{0}C {1}A {2}  {3}", (print.Request.ChequeFrom + (currentChequeNo - 1)).ToString("000000"), print.Request.MICRCode, new string(' ', 6), print.Request.TransactionCode);
            else
        ...
        {
            //var lastDate = new DateTime(2070, 11, 20);
        ...
        private static void PrintBlankRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            e.Graphics.DrawString("Sunday Working : " + "SUNDAY CLOSE", font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.2f));
            ...
            e.Graphics.DrawString(string.Format("Branch Service: 2nd & 4th SAT IS HOLYDAY"), font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 4.6f));
            /* END Left Section */
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            Font font8Bold = new Font("Verdana", 8, FontStyle.Bold);
            Font font9Bold = new Font("Verdana", 9, FontStyle.Bold);
            Font font10Bold = new Font("Verdana", 10, FontStyle.Bold);
            Font font5 = new Font("Verdana", 5);
            ...
            if (IsPayOrder(section.PrintRequest.Request.TransactionCode) && BatchInPrinting.ChequeLayout.custom1Visible)
            {
                e.Graphics.DrawString(BatchInPrinting.ChequeLayout.custom1Text, font10Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.custom1X), y + GetDPI(BatchInPrinting.ChequeLayout.custom1Y));
            }
            ...
            if (BatchInPrinting.ChequeLayout.ifscVisble)
                e.Graphics.DrawString(string.Format("RTGS/NEFT IFSC CODE: {0}", section.PrintRequest.Branch.IFSC), IFSCFont, brush, x + GetDPI(BatchInPrinting.ChequeLayout.ifscX), y + GetDPI(BatchInPrinting.ChequeLayout.ifscY));
            ...
            {
                if (IsPayOrder(section.PrintRequest.Request.TransactionCode))
                    e.Graphics.DrawString(string.Format("{0}:", "Not Over Rs."), font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY));
                else
            ...
            if (IsPayOrder(section.PrintRequest.Request.TransactionCode) && BatchInPrinting.ChequeLayout.custom2Visible)
            {
                e.Graphics.DrawString(string.Format("{0} ______________________________", BatchInPrinting.ChequeLayout.custom2Text), font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.custom2X), y + GetDPI(BatchInPrinting.ChequeLayout.custom2Y));
            }
            ...
            if (IsPayOrder(section.PrintRequest.Request.TransactionCode) && BatchInPrinting.ChequeLayout.accountPayeeVisble)
            {
