...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        private static string GetMICR(PrintRequest print, int currentChequeNo)
        {
            ...
            // For Pay Order (TransactionCode = 12), keep 6 spaces
            if (print.Request.TransactionCode == 12)
            {
                accountPart = "      "; // exactly 6 spaces
            }
            else
            {
                accountPart = Convert.ToInt32(
        ...
        private static Image GetBarcode(string number, int barHeight, RotateFlipType rotateFlipType)
        {
            if (string.IsNullOrWhiteSpace(number))
                return null;   // or return a blank image if required
        ...
        private static void PrintRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.RequestLayout.barcode1Visble && barcode != null)
            {
                e.Graphics.DrawImage(
                    barcode,
                    x + GetDPI(BatchInPrinting.RequestLayout.barcode1X),
                    y + GetDPI(BatchInPrinting.RequestLayout.barcode1Y)
                );
            }
        ...
        private static void PrintBlankRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.RequestLayout.nameAddress1Visble)
            {
                font6 = new Font(font6, FontStyle.Bold);
            ...
            Font ont6Bold = new Font(font6, FontStyle.Bold);
            ...
            e.Graphics.DrawString("Holidays: " + section.PrintRequest.Branch.Time2+"    2 & 4 Saturday, Sunday", font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 1.8f));
            ...
            e.Graphics.DrawString(string.Format("Contact No.: {0}, {1}", section.PrintRequest.Branch.Telephone1, section.PrintRequest.Branch.Telephone2),font6,brush,x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X),y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.6f)
            );
            ...
            e.Graphics.DrawString("CHEQUE BOOK REQUEST (BANK COPY)", font8Bold, brush, x + GetDPI(17.5f), y + GetDPI(0.5f));
            e.Graphics.DrawString("THE BR.MANAGER", font7Bold, brush, x + GetDPI(15f), y + GetDPI(1.1f));
            ...
            e.Graphics.DrawString("Signature Verified / Authorized", font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X + 6.9f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 2.1f));
            e.Graphics.DrawString("Signature of Bearer", font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 4.2f));

            e.Graphics.DrawString("PROPRIETOR", font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X + 7.5f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 4.9f));

            RectangleF stampRect = new RectangleF(x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X - 4.5f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 1f), 450, 110);
            ...
            if (BatchInPrinting.RequestLayout.audiText1Visble)
                e.Graphics.DrawString(auditText, font7, brush, new PointF((-y) + (-auditTextSize.Width) + (-GetDPI(BatchInPrinting.RequestLayout.audiText1Y)), x + GetDPI(BatchInPrinting.RequestLayout.audiText1X)));
            //BatchInPrinting.RequestLayout.audiText1X=0.05f

            if (BatchInPrinting.RequestLayout.audiText2Visble)
                e.Graphics.DrawString(auditText, font7, brush, new PointF((-y) + (-auditTextSize.Width) + (-GetDPI(BatchInPrinting.RequestLayout.audiText2Y)), x + GetDPI(BatchInPrinting.RequestLayout.audiText2X)));
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            if (section.PrintRequest.Request.TransactionCode == 12)
            {
                e.Graphics.DrawString(
                    "Not over ____________",
                    font9,
                    brush,
                    x + GetDPI(BatchInPrinting.ChequeLayout.custom1X + 8.5f),
                    y + GetDPI(BatchInPrinting.ChequeLayout.custom1Y + 8f)
                );
            }
            ...
            if (BatchInPrinting.ChequeLayout.accountNoVisble)
            {
                ...
                if (section.PrintRequest.Request.TransactionCode == 12)
                {
                    label = " "; // or string.Empty
                    value = "On Account Of ________________";
                }
                else
                {
                    label = section.PrintRequest.AccountType.Name;
                    value = section.PrintRequest.Request.AccountNoFull;
                }
                ...
                if (section.PrintRequest.Request.TransactionCode == 12)
                {
                    e.Graphics.DrawString(
                        ...
                        font8Bold,
                        brush,
                        x + GetDPI(BatchInPrinting.ChequeLayout.custom4X + 18.5f),
                        y + GetDPI(BatchInPrinting.ChequeLayout.custom4Y + 1.5f)
                    );
                }
            ...
            if (BatchInPrinting.ChequeLayout.stampVisble)
            {
                RectangleF stampRect = new RectangleF(
                    ...
                    y + GetDPI(BatchInPrinting.ChequeLayout.stampY),
                    450,
                    110
                );
                ...
                // 🔹 Move Custom 2,3,4 HERE (only for TransactionCode == 12)
                if (section.PrintRequest.Request.TransactionCode == 12)
                {
                    // Custom2 – Code No.
                    e.Graphics.DrawString(
                        ...
                        font8Bold,
                        brush,
                        x + GetDPI(BatchInPrinting.ChequeLayout.custom2X + 16f),
                        y + GetDPI(BatchInPrinting.ChequeLayout.custom2Y + 8.9f)
                    );
                    ...
                    e.Graphics.DrawString(
                        ...
                        font8Bold,
                        brush,
                        x + GetDPI(BatchInPrinting.ChequeLayout.custom3X + 22f),
                        y + GetDPI(BatchInPrinting.ChequeLayout.custom3Y + 8.8f)
                    );

                    // Custom4 – Authorized Signatory
                    e.Graphics.DrawString(
                        "Authorized Signatory",
                        font8Bold,
                        brush,
                        x + GetDPI(BatchInPrinting.ChequeLayout.custom4X + 15.8f),
                        y + GetDPI(BatchInPrinting.ChequeLayout.custom4Y + 9.3f)
                    );
            ...
            string.Format(
                ...
                new string((section.PrintRequest.Request.AccountNo ?? string.Empty).Where(char.IsDigit).ToArray()).PadLeft(6, '0').Substring(Math.Max(0, new string((section.PrintRequest.Request.AccountNo ?? string.Empty).Where(char.IsDigit).ToArray()).Length - 6)),
