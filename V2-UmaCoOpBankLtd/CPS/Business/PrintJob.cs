    ...
    public class PrintJob
    {
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y, float width, float height)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.Address2))
                sb.Append(" " + request.Address2);
            if (!string.IsNullOrWhiteSpace(request.Address3))
                sb.Append(" " + request.Address3);
            if (!string.IsNullOrWhiteSpace(request.City))
                sb.Append(string.Format(" {0}, {1}", request.City, request.PostalCode));
        ...
        {
            // Extract only digits from AccountNo
            string digitsOnly = new string(print.Request.AccountNo
                .Where(char.IsDigit)
                .ToArray());
            ...
            int accountSuffix = 0;
            int.TryParse(last6, out accountSuffix);

            // 🔥 Key condition
            string accountPart = print.Request.TransactionCode == 12
                ? "      "
                : accountSuffix.ToString("000000");

            return string.Format("C{0}C {1}A {2}C {3}",
                (print.Request.ChequeFrom + (currentChequeNo - 1)).ToString("000000"),
                print.Branch.MICR,
                accountPart,
                print.Request.TransactionCode);
        }
        // ONLY FOR CHEQUE PRINTING
        //private static string GetPrintTransactionCode(int originalCode)
        //{
        // switch (originalCode)
        // {
        // case 10: return "31"; // Savings
        // case 11: return "29"; // Current
        // case 13: return "30"; // CC
        // case 12: return "12"; // Pay Order
        //default: return originalCode.ToString();
        // }
        // }

        //private static string GetMICRForPrint(PrintRequest print, int currentChequeNo)
        //{
        //string txnCode = GetPrintTransactionCode(print.Request.TransactionCode);

        // return string.Format("C{0}C {1}A {2}C {3}",
        //(print.Request.ChequeFrom + (currentChequeNo - 1)).ToString("000000"),
        // print.Branch.MICR,
        // Convert.ToInt32(print.Request.AccountNo.Substring(Math.Max(0, print.Request.AccountNo.Length - 6))).ToString("000000"),
        // TransactionCode);
        //}
        ...
        {
            //var lastDate = new DateTime(2023, 4, 15);
        ...
        private static void PrintRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.RequestLayout.branchAddress1Visble)
                e.Graphics.DrawString(section.PrintRequest.Branch.Name, font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.branchAddress1X), y + GetDPI(BatchInPrinting.RequestLayout.branchAddress1Y));

            if (BatchInPrinting.RequestLayout.branchAddress2Visble)
                e.Graphics.DrawString(section.PrintRequest.Branch.Name, font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.branchAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.branchAddress2Y));
        ...
        private static void PrintBlankRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.RequestLayout.accountNo1Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.AccountNoFull), font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo1X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo1Y));
            ...
            if (!string.IsNullOrEmpty(section.PrintRequest.Request.mob))
            {
                e.Graphics.DrawString(
                    "Contact No: " + section.PrintRequest.Request.mob,
                    font7Bold,
                    brush,
                    x + GetDPI(1f),   // Adjust X position
                    y + GetDPI(6f)    // Adjust Y position
                );
            }
            ...
            // Shift values
            e.Graphics.DrawString("BRANCH INFORMATION", font8Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X  +2.5f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 1f));
            e.Graphics.DrawString("Regu. Busi. Hrs.: Mon to Sat 10 to 4 " + section.PrintRequest.Branch.Time1, font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X +1.5f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 1.5f));
            e.Graphics.DrawString("Half Day Busi. Hrs.: " + section.PrintRequest.Branch.Time1, font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 1.5f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 1.9f));
            e.Graphics.DrawString("Holidays: Sunday & 2nd & 4Th Sat" + section.PrintRequest.Branch.Time2, font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 1.5f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.3f));
            e.Graphics.DrawString(string.Format("Branch Email : {0}", section.PrintRequest.Branch.Email), font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 1.5f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.7f));
            e.Graphics.DrawString(string.Format("Contact No. : {0}", section.PrintRequest.Branch.Telephone1), font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 1.5f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 3.1f));
            e.Graphics.DrawString(string.Format("Web Address : {0}", oBankMasterDTO.WebAddress), font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 1.5f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 3.5f));
            /* END Left Section */

            /* START Right Section */

            e.Graphics.DrawString("CHEQUE BOOK REQUEST (BANK COPY)", font8Bold, brush, x + GetDPI(12.5f), y + GetDPI(0.1f));
            e.Graphics.DrawString("THE MANAGER", font7Bold, brush, x + GetDPI(11f), y + GetDPI(0.75f));
            ...
            e.Graphics.DrawString("Date: _____________", font7Bold, brush, x + GetDPI(23.5f), y + GetDPI(2.25f));

            if (BatchInPrinting.RequestLayout.chequeFrom2Visble)
                e.Graphics.DrawString(string.Format("Cheque no. From: {0} To {1}", section.PrintRequest.Request.ChequeFrom.ToString("000000"), section.PrintRequest.Request.ChequeTo.ToString("000000")), font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2X + 2f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2Y + 1f));
            //GetDPI(12.3f), GetDPI(3.6f)
            
            e.Graphics.DrawString("Please issue _________Cheque books containig_________Leaves", font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2X- 7.5f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2Y + 1.5f));
            e.Graphics.DrawString("Characteristics:      Bearer     Order", font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2X-7.5f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2Y + 5.5f));
            Pen blackPen = new Pen(Color.FromArgb(255, 0, 0, 0), 1);
            //e.Graphics.DrawRectangle(blackPen, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2X + 1f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2Y + 4.5f), 15, 15);
            //e.Graphics.DrawRectangle(blackPen, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2X + 3f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2Y + 4.5f), 15, 15);
            e.Graphics.DrawString("Cheque Book :         Normal         Payable At Par", font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2X-7.5f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2Y + 5.9f));
           // e.Graphics.DrawRectangle(blackPen, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2X + 1f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2Y + 5f), 15, 15);
           // e.Graphics.DrawRectangle(blackPen, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2X + 3f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2Y + 5f), 15, 15);
           ...
           // e.Graphics.DrawString(string.Format("Tel No: {0}", section.PrintRequest.Request.telr), font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 2.5f));
            e.Graphics.DrawString(string.Format("Contact No: {0}", section.PrintRequest.Request.mob), font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X -3f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 3.5f));
            e.Graphics.DrawString("Signature Verified / Authorized", font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X + 6.75f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 3f));
            e.Graphics.DrawString("Signature of Bearer", font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X -3f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 5f));

            RectangleF stampRect = new RectangleF(x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X - 4.5f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 2f), 450, 110);
            ...
            // Draw the text and the surrounding rectangle.
            string stampText = GetStamp1(section.PrintRequest.Request);
            string[] lines = stampText.Split(new[] { Environment.NewLine }, StringSplitOptions.None);

            float lineHeight = font8Bold.GetHeight(e.Graphics);

            float stampX = stampRect.Right - 5;

            // 🔥 Move block slightly DOWN (increase this value if needed)
            float verticalOffset = 30f;

            float stampY = stampRect.Bottom - (lines.Length * lineHeight) + verticalOffset;

            for (int i = 0; i < lines.Length; i++)
            {
                string line = lines[i];

                SizeF textSize = e.Graphics.MeasureString(line, font8Bold);

                e.Graphics.DrawString(
                    line,
                    font8Bold,
                    brush,
                    stampX - textSize.Width,
                    stampY
                );

                if (i < lines.Length - 1 && !string.IsNullOrWhiteSpace(lines[i + 1]))
                    stampY += lineHeight + 2f;
                else
                    stampY += lineHeight;
            }
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            if (BatchInPrinting.ChequeLayout.ifscVisble)
                e.Graphics.DrawString(string.Format("RTGS/NEFT IFSC: {0}", section.PrintRequest.Branch.IFSC), font8Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.ifscX), y + GetDPI(BatchInPrinting.ChequeLayout.ifscY));
            ...
            {
                //   Font accFont = new Font(font5.FontFamily, 10, FontStyle.Regular); 
                Font accFont = new Font("Arial Narrow", 10, FontStyle.Bold);
                e.Graphics.DrawString(
                    string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.AccountNoFull),
                    accFont,
                    brush,
                    x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX),
                    y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY)
                );
            }
            // If Account Type is Pay Order
            if (section.PrintRequest.Request.TransactionCode == 12)
            {
                // Custom3 → PAY ORDER
                if (BatchInPrinting.ChequeLayout.custom3Visible)
                {
                    e.Graphics.DrawString(
                        "PAY ORDER",
                        font9Bold,
                        brush,
                        x + GetDPI(BatchInPrinting.ChequeLayout.custom3X + 1.5f),
                        y + GetDPI(BatchInPrinting.ChequeLayout.custom3Y - 0.25f)
                    );
                }

                // Custom5 → Not Over Rs.
                if (BatchInPrinting.ChequeLayout.custom5Visible)
                {
                    e.Graphics.DrawString(
                        "Not Over Rs. _______________",
                        font9Bold,
                        brush,
                        x + GetDPI(BatchInPrinting.ChequeLayout.custom5X + 2f),
                        y + GetDPI(BatchInPrinting.ChequeLayout.custom5Y + 1.35f)
                    );
                }

                // Custom2 → Name Of Purchaser
                if (BatchInPrinting.ChequeLayout.custom2Visible)
                {
                    Font smallFont = new Font(font7Bold.FontFamily, 6, FontStyle.Bold);

                    e.Graphics.DrawString(
                        "Name Of Purchaser___________________________",
                        font9Bold,
                        brush,
                        x + GetDPI(BatchInPrinting.ChequeLayout.custom2X + 0.5f),
                        y + GetDPI(BatchInPrinting.ChequeLayout.custom2Y - 0.3f)
                    );
                }
            }
            else
            {
                // Default → PAYABLE AT PAR
                if (BatchInPrinting.ChequeLayout.custom2Visible)
                {
                    Font smallFont = new Font(font7Bold.FontFamily, 6, FontStyle.Bold);
                    ...
                    e.Graphics.DrawString(
                        ...
                        smallFont,
                        brush,
                        x + GetDPI(BatchInPrinting.ChequeLayout.custom2X),
                        y + GetDPI(BatchInPrinting.ChequeLayout.custom2Y + 1.2f)
                    );
                }
            }
            if (BatchInPrinting.ChequeLayout.stampVisble)
            {
                float stampY = BatchInPrinting.ChequeLayout.stampY;

                // 🔥 Move UP for Txn = 12
                if (section.PrintRequest.Request.TransactionCode == 12)
                {
                    stampY -= 1f; // adjust this value if needed (0.2–0.5)
                }

                // Stamp text
                string stampText = section.PrintRequest.Request.TransactionCode == 12
                    ? "For UMA CO-OPERATIVE BANK LTD.\n\n\n\n\nOfficer                               Manager"
                    : GetStamp(section.PrintRequest.Request);

                // Proprietor shift only for normal cases
                if (section.PrintRequest.Request.TransactionCode != 12 &&
                    !string.IsNullOrEmpty(stampText) &&
                    stampText.ToUpper().Contains("PROPRIETOR"))
                {
                    stampY += 1f;
                }
                ...
                e.Graphics.DrawString(stampText, font8Bold, brush, stampRect, stringFormat);
            }
            ...
            // Extract only digits from AccountNo
            string digitsOnly = new string(section.PrintRequest.Request.AccountNo
                .Where(char.IsDigit)
                .ToArray());

            // Take last 6 digits (or fewer if not available)
            string last6 = digitsOnly.Length >= 6
                ? digitsOnly.Substring(digitsOnly.Length - 6)
                : digitsOnly;
