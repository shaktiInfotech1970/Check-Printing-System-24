...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
            ...
            for (int i = 0; i < requestGroup.Requests.Count(); i += 3)
            {
                ...
                if (requests.Count() < 3)
                {
                    foreach (var r in requests)
                    {
                        ...
                        batch.RequestPage = GetRequestPage(r); // GetRequestPage(requests);
        ...
        private static string GetBranchAddress(BranchMasterDTO branch)
        {
            ...
            //No need to print branch name as per the cheque image sent by bank
        ...
        private static string GetBranchAddress1(BranchMasterDTO branch)
        {
            ...
            if (!string.IsNullOrWhiteSpace(branch.Name))
                sb.Append("SHRI CHHATRAPATI RAJARSHI SHAHU URBAN CO-OP. BANK LTD., BEED");
        ...
        private static Image GetAccountPayeeImage()
        {
            ...
            return (Image)CPS.Properties.Resources.ACPayeeSleeping;
        }
        ...
        private static PrintDocument RequestPrintDocument = null;
        private static PrintDocument InnerRequestPrintDocument = null;
        private static PrintDocument ChequePrintDocument = null;
        ...
        private static bool SendToPrinter(PrinterPreference printerPreference, PrinterJob printerJob, PrintType printType)
        {
            if (printerPreference != null)
            {
                ...
                foreach (var batch in printerJob.PrintBatches)
                {
                    ...
                    {
                        RequestPrintDocument = GetPrintDocument(printerPreference.Name, printerPreference.RequestTray, new PrintPageEventHandler(requestPrintDocument_PrintPage));
                        InnerRequestPrintDocument = GetPrintDocument(printerPreference.Name, printerPreference.RequestTray, new PrintPageEventHandler(innerRequestPrintDocument_PrintPage));
                        RequestPrintDocument.Print();
                        if (InnerRequestPrintDocument != null && printType == PrintType.RePrintRequest)
                            InnerRequestPrintDocument.Print();
                    }
                    else
                    {
                        InnerRequestPrintDocument = null;
                    }
                    if (printType != PrintType.Request && printType != PrintType.RePrintRequest)
                    {
                        ChequePrintDocument = GetPrintDocument(printerPreference.Name, printerPreference.ChequeTray, new PrintPageEventHandler(chequePrintDocument_PrintPage));
        ...
        {
            if (BatchInPrinting.PrintBatch.RequestPage.Sections.Count == 1)
            {
                PrintRequestSection(e, BatchInPrinting.PrintBatch.RequestPage.Sections[0], 1);
                PrintInnerRequestSection(e, BatchInPrinting.PrintBatch.RequestPage.Sections[0], 3);
                InnerRequestPrintDocument = null;
            }
            else
            {
                ...
                {
                    PrintRequestSection(e, BatchInPrinting.PrintBatch.RequestPage.Sections[i - 1], i);
                }
            }
        }

        private static void innerRequestPrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            for (int i = 1; i <= BatchInPrinting.PrintBatch.RequestPage.Sections.Count; i++)
                PrintInnerRequestSection(e, BatchInPrinting.PrintBatch.RequestPage.Sections[i - 1], i);
        }

        private static void PrintRequestSection(PrintPageEventArgs e, PrintSection section, int sectionNo)
        {
            SolidBrush brush = new SolidBrush(Color.Black);
            ...
            /* START Left Section */
            e.Graphics.DrawString(GetBranchAddress1(section.PrintRequest.Branch), font6Bold, brush, x + GetDPI(0.2f), y + GetDPI(0.2f));
            e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.AccountNoFull), font7Bold, brush, x + GetDPI(1.4f), y + GetDPI(2.1f));
            PrintNameAddress(e, section.PrintRequest.Request, x + GetDPI(1.4f), y + GetDPI(2.5f));
            ...
            e.Graphics.DrawString(string.Format("Cheque no. From: {0} To {1}", section.PrintRequest.Request.ChequeFrom.ToString("000000"), section.PrintRequest.Request.ChequeTo.ToString("000000")), font8Bold, brush, x + GetDPI(0.5f), y + GetDPI(7.5f));

            //e.Graphics.DrawString("BRANCH INFORMATION", font8Bold, brush, x + GetDPI(1.5f), y + GetDPI(8.3f));
            //e.Graphics.DrawString("Regu. Busi.Hrs.: " + section.PrintRequest.Branch.Time1, font6, brush, x + GetDPI(0.5f), y + GetDPI(8.9f));
            //e.Graphics.DrawString("Holidays: " + section.PrintRequest.Branch.Time2, font6, brush, x + GetDPI(0.5f), y + GetDPI(9.3f));
            //e.Graphics.DrawString(string.Format("Branch Email : {0}", section.PrintRequest.Branch.Email), font6, brush, x + GetDPI(0.5f), y + GetDPI(9.7f));
            //e.Graphics.DrawString(string.Format("Contact No. : {0}", section.PrintRequest.Branch.Telephone1), font6, brush, x + GetDPI(0.5f), y + GetDPI(10.1f));
            //e.Graphics.DrawString("Web Address: www.shahubank.com", font6, brush, x + GetDPI(0.5f), y + GetDPI(10.5f));
            /* END Left Section */
            ...
            e.Graphics.DrawString(GetBranchAddress1(section.PrintRequest.Branch), font7Bold, brush, x + GetDPI(12.3f), y + GetDPI(1.8f));
            //e.Graphics.DrawString("Date: ____________________", font7Bold, brush, x + GetDPI(21f), y + GetDPI(1.3f));
            e.Graphics.DrawString(string.Format("Cheque no. From: {0} To {1}", section.PrintRequest.Request.ChequeFrom.ToString("000000"), section.PrintRequest.Request.ChequeTo.ToString("000000")), font7Bold, brush, x + GetDPI(12.4f), y + GetDPI(4.1f));

            //e.Graphics.DrawString("Please issue _________Cheque books containig_________Leaves", font7Bold, brush, x + GetDPI(12.4f), y + GetDPI(4.7f));
            //e.Graphics.DrawString("Characteristics:          Bearer        Order", font7Bold, brush, x + GetDPI(12.4f), y + GetDPI(5.4f));
            //Pen blackPen = new Pen(Color.FromArgb(255, 0, 0, 0), 1);
            //e.Graphics.DrawRectangle(blackPen, x + GetDPI(18.1f), y + GetDPI(5.35f), 15, 15);
            //e.Graphics.DrawRectangle(blackPen, x + GetDPI(20.1f), y + GetDPI(5.35f), 15, 15);
            ...
            //e.Graphics.DrawString("Cheque Book :             Normal                    Payable At Par", font7Bold, brush, x + GetDPI(12.4f), y + GetDPI(6.1f));
            //e.Graphics.DrawRectangle(blackPen, x + GetDPI(18.35f), y + GetDPI(6.05f), 15, 15);
            //e.Graphics.DrawRectangle(blackPen, x + GetDPI(23.5f), y + GetDPI(6.05f), 15, 15);
            e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.AccountNoFull), font8Bold, brush, x + GetDPI(12.4f), y + GetDPI(6.6f));
            PrintNameAddress(e, section.PrintRequest.Request, x + GetDPI(12.4f), y + GetDPI(7.2f));
            e.Graphics.DrawString(string.Format("Tel No: {0}", section.PrintRequest.Request.telr), font7, brush, x + GetDPI(12.4f), y + GetDPI(9.5f));
            e.Graphics.DrawString(string.Format("Mobile No: {0}", section.PrintRequest.Request.mob), font7, brush, x + GetDPI(12.4f), y + GetDPI(10.0f));
            //e.Graphics.DrawString("Signature Verified / Authorized", font6, brush, x + GetDPI(22.3f), y + GetDPI(8.4f));
            //e.Graphics.DrawString("Signature of Bearer", font6Bold, brush, x + GetDPI(12.4f), y + GetDPI(11.0f)); //11.06

            RectangleF stampRect = new RectangleF(x + GetDPI(11.2f), y + GetDPI(7.4f), 450, 110);
        ...
        private static void PrintInnerRequestSection(PrintPageEventArgs e, PrintSection section, int sectionNo)
        {
            SolidBrush brush = new SolidBrush(Color.Black);
            Font font9 = new Font("Verdana", 9);
            Font font8 = new Font("Verdana", 8);
        ...
        private static void chequePrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            if (printPage != null)
            {
                ...
                if (nextPage != null)
                {
                    if (InnerRequestPrintDocument != null && nextPage.PageNo + 4 == BatchInPrinting.PrintBatch.EndPage)
                        InnerRequestPrintDocument.Print();
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            if (BatchInPrinting.ChequeLayout.ifscVisble)
                e.Graphics.DrawString(string.Format("IFSC CODE: {0}", section.PrintRequest.Branch.IFSC), font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.ifscX), y + GetDPI(BatchInPrinting.ChequeLayout.ifscY));

            if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder))
            {
                RectangleF stampRect = new RectangleF(x, y + GetDPI(1.4f), 525, 10);
                StringFormat stringFormat = new StringFormat();
                stringFormat.Alignment = StringAlignment.Far;
            ...
            if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder))
            {
                e.Graphics.DrawString("NOT OVER RS.:__________________", font9, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX + 5.8f), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY + 1.8f));
            }

            e.Graphics.DrawString("CBS BRANCH", font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY + 2.9f));
