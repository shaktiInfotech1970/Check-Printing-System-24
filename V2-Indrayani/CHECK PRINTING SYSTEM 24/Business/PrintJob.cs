...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        private static string GetMICR(RequestDTO request, int currentChequeNo)
        {
            ...
            // Pay Order → always blank
            if (request.TransactionCode == 12)
            {
                accPart = new string(' ', 6);
            }
            else
            {
                string accDigits = new string(
                    (request.AccountNo ?? string.Empty)
                        .Where(char.IsDigit)
                        .ToArray()
                );

                if (string.IsNullOrEmpty(accDigits))
                {
                    accPart = new string(' ', 6);
                }
                else if (accDigits.Length >= 6)
                {
                    accPart = accDigits.Substring(accDigits.Length - 6);
                }
                else
                {
                    // less than 6 digits → LEFT pad with ZEROES
                    accPart = accDigits.PadLeft(6, '0');
                }
            }

            return string.Format(
                "C{0}C {1}A {2}C {3}",
                (request.ChequeFrom + (currentChequeNo - 1)).ToString("000000"),
                request.MICRCode,
                accPart,
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            Font IFSCFont = new Font("Consolas", 10, FontStyle.Bold);
            ...
            if (BatchInPrinting.ChequeLayout.branchAddressVisble)
                e.Graphics.DrawString(GetBranchAddress(section.PrintRequest.Branch), font8, brush, x + GetDPI(BatchInPrinting.ChequeLayout.branchAddressX), y + GetDPI(BatchInPrinting.ChequeLayout.branchAddressY));
    ...
    string.Format(
        ...
        new string(section.PrintRequest.Request.AccountNo.Where(char.IsDigit).ToArray())
            .PadLeft(6, '0')
            .Substring(
                Math.Max(0,
                new string(section.PrintRequest.Request.AccountNo.Where(char.IsDigit).ToArray()).Length - 6)
            ),
