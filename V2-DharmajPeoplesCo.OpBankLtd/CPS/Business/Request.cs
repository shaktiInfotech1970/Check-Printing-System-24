        ...
        public override bool IsValid(IList<ValidationResult> results)
        {
            ...
            try
            {
                using (var context = new CPSDbContext())
                {
                    ...
                    if (repository.FilterBy(w => w.Id != this.Id && w.BranchId == this.BranchId && w.additional_f10 == this.additional_f10 && (((w.ChequeFrom >= this.ChequeFrom) && (w.ChequeFrom <= this.ChequeTo)) || ((this.ChequeFrom >= w.ChequeFrom) && (this.ChequeFrom <= w.ChequeTo)))).FirstOrDefault() != null)
                    {
                        errorMsg = string.Format("[{0}-{1}] Cheque series are already exist", this.ChequeFrom, this.ChequeTo);
                        errorMsg += string.IsNullOrEmpty(this.additional_f10) ? "." : string.Format(" for account no {0}.", this.additional_f10);
                        results.Add(new ValidationResult(errorMsg));
                    }
                    if (repository.FilterBy(w => w.Id != this.Id && w.BranchId == this.BranchId && w.additional_f10 == this.additional_f10 && w.TransactionCode != this.TransactionCode).FirstOrDefault() != null)
                    {
