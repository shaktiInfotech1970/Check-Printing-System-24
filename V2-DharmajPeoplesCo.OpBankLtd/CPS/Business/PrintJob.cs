...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        {
            return string.Format("C{0}C {1}A {2}C {3}", (print.Request.ChequeFrom + (currentChequeNo - 1)).ToString("000000"), print.Branch.MICR, Convert.ToInt32(print.Request.AccountNoFull.Substring(Math.Max(0, print.Request.AccountNoFull.Length - 6))).ToString("000000"), print.Request.TransactionCode);
        }
        ...
        private static void PrintRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.RequestLayout.accountNo1Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.additional_f10), font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo1X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo1Y));

            if (BatchInPrinting.RequestLayout.accountNo2Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.additional_f10), font9Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo2X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo2Y));

            var barcode = GetBarcode(section.PrintRequest.Request.additional_f10, 15, System.Drawing.RotateFlipType.RotateNoneFlipNone);
        ...
        private static void PrintBlankRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.RequestLayout.accountNo1Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.additional_f10), font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo1X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo1Y));
            ...
            if (BatchInPrinting.RequestLayout.accountNo2Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.additional_f10), font8Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo2X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo2Y));
        ...
        private static void chequePrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            if (printPage != null)
            {
                ...
                var printHistoryDTO = PrintHistoryDTO.GetPrintHistory(printPage.Sections[0].PrintRequest.Request.PrintJobNo ?? 0, printPage.Sections[0].PrintRequest.Request.additional_f10);
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            if (BatchInPrinting.ChequeLayout.accountNoVisble)
            {
                ...
                if (section.PrintRequest.Request.TransactionCode == 13)
                {
                    if (string.Equals(section.PrintRequest.Request.additional_f1, "CC", StringComparison.OrdinalIgnoreCase))
                    {
                        // Case: TxnCode 13 with CC
                        finalText = string.Format("CC A/C No : {0}", section.PrintRequest.Request.additional_f10);
                    }
                    else
                    {
                        // Case: TxnCode 13 but not CC
                        finalText = string.Format("A/C No : {0}", section.PrintRequest.Request.additional_f10);
                    }
                }
                else
                {
                    // Other txn codes (10, 11, 12, …)
                    finalText = string.Format("{0}  : {1}",
                        section.PrintRequest.AccountType.Name,
                        section.PrintRequest.Request.additional_f10);
                }
            ...
            var imgBarcode = GetBarcode(string.Format("{0}{1}{2}{3}", (section.PrintRequest.Request.ChequeFrom + (section.SequenceNo - 1)).ToString("000000"), section.PrintRequest.Request.MICRCode, Convert.ToInt32(section.PrintRequest.Request.AccountNoFull).ToString("000000"), section.PrintRequest.Request.TransactionCode), 12, System.Drawing.RotateFlipType.Rotate270FlipNone);
