...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        private static void GetBranchAddress1(PrintPageEventArgs e, BranchMasterDTO branch, string bankName, int fontsize, float x, float y)
        {
            SolidBrush brush = new SolidBrush(Color.Black);
            Font font7 = new Font("Verdana", fontsize);
            Font font6 = new Font("Verdana", fontsize - 1);

            // Bank Name           
            if (!string.IsNullOrWhiteSpace(bankName))
                e.Graphics.DrawString(bankName, font7, brush, x, y);
            ...
            if (!string.IsNullOrWhiteSpace(branch.Name))
            {
                e.Graphics.DrawString(string.Format("{0}:", branch.Name), font7, brush, x, y);
                stringSize = e.Graphics.MeasureString(string.Format("{0}:", branch.Name), font7);
            }

            var sameLineAddress = new StringBuilder();
            if (!string.IsNullOrWhiteSpace(branch.AddressLine1))
                sameLineAddress.Append(branch.AddressLine1);
            if (!string.IsNullOrWhiteSpace(branch.AddressLine2))
                sameLineAddress.Append(", " + branch.AddressLine2);

            e.Graphics.DrawString(sameLineAddress.ToString(), font6, brush, x + stringSize.Width, y);

            y += 14;

            sameLineAddress.Clear();
            if (!string.IsNullOrWhiteSpace(branch.AddressLine3))
                sameLineAddress.Append(branch.AddressLine3 + ", ");
            if (!string.IsNullOrWhiteSpace(branch.City))
                sameLineAddress.Append(string.Format("{0} - {1}", branch.City, branch.PostalCode));

            e.Graphics.DrawString(sameLineAddress.ToString(), font6, brush, x, y);
        ...
        private static string GetStamp(RequestDTO request)
        {
            ...
            {
                sb.Append(string.Format("For {0}", BankMasterDTO.GetBankName()));
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(request.JointName1))
                        sb.Append(" / " + request.JointName1);

                    if (!string.IsNullOrWhiteSpace(request.JointName2))
                        sb.Append(" / " + request.JointName2);                    
                }
            }
            return sb.ToString();
        }
        ...
        {
            //var lastDate = new DateTime(2023, 2, 10);
        ...
        private static void PrintBlankRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            /* START Left Section */
            if (BatchInPrinting.RequestLayout.branchAddress1Visble)
                GetBranchAddress1(e, section.PrintRequest.Branch, oBankMasterDTO?.Name, 7, x + GetDPI(BatchInPrinting.RequestLayout.branchAddress1X), y + GetDPI(BatchInPrinting.RequestLayout.branchAddress1Y));
            //GetDPI(0.2f), GetDPI(0.2f)

            if (BatchInPrinting.RequestLayout.accountNo1Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.AccountNoFull), font8, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo1X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo1Y));
            ...
            if (BatchInPrinting.RequestLayout.chequeFrom1Visble)
                e.Graphics.DrawString(string.Format("Present Cheque no. From: {0} To {1}", section.PrintRequest.Request.ChequeFrom.ToString("000000"), section.PrintRequest.Request.ChequeTo.ToString("000000")), font8, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y));
            //GetDPI(0.5f), GetDPI(7.5f)
            e.Graphics.DrawString("BRANCH INFORMATION", font9, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 0.8f));
            e.Graphics.DrawString("Regu. Busi. Hrs.: " + section.PrintRequest.Branch.Time1, font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 1.4f));
            e.Graphics.DrawString("Half. Day Busi. Hrs.: " + section.PrintRequest.Branch.Time2, font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 1.8f));
            e.Graphics.DrawString("Weekly Holiday : " + "SUNDAY", font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.2f));
            e.Graphics.DrawString(string.Format("Branch Email-1 : {0}", section.PrintRequest.Branch.Email), font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.6f));
            e.Graphics.DrawString(string.Format("Branch Email-2 : {0}", section.PrintRequest.Branch.Email2), font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 3f));
            e.Graphics.DrawString(string.Format("Web Address : {0}", oBankMasterDTO.WebAddress), font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 3.4f));
            e.Graphics.DrawString(string.Format("Contact No.: {0} / {1}", section.PrintRequest.Branch.Telephone1, section.PrintRequest.Branch.Telephone2), font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 3.8f));
            /* END Left Section */
            ...
            if (BatchInPrinting.RequestLayout.branchAddress2Visble)
                GetBranchAddress1(e, section.PrintRequest.Branch, oBankMasterDTO?.Name, 8, x + GetDPI(BatchInPrinting.RequestLayout.branchAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.branchAddress2Y));
            //GetDPI(12.3f), GetDPI(1.8f)


            e.Graphics.DrawString("Please issue me_________Cheque book(s) containig_________Leaves", font8, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2X), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2Y - 1f));
            e.Graphics.DrawString("by Register Post/bearer", font8, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2X), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2Y - 0.5f));
            if (BatchInPrinting.RequestLayout.chequeFrom2Visble)
                e.Graphics.DrawString(string.Format("Present Cheque no. From: {0} To {1}", section.PrintRequest.Request.ChequeFrom.ToString("000000"), section.PrintRequest.Request.ChequeTo.ToString("000000")), font9, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2X), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom2Y));
            //GetDPI(12.3f), GetDPI(3.6f)

            if (BatchInPrinting.RequestLayout.accountNo2Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.AccountNoFull), font9, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo2X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo2Y));
            ...
            e.Graphics.DrawString("Signature Verified / Authorized", font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X + 9.9f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 2.2f));
            e.Graphics.DrawString("Signature of Bearer", font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 2.2f));
            e.Graphics.DrawString("Received By", font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 3.3f));
            ...
            // Draw the text and the surrounding rectangle.
            e.Graphics.DrawString(GetStamp1(section.PrintRequest.Request), font9, brush, stampRect, stringFormat);

            /* END Right Section*/

            /* START Audit Text */
            var auditText = GetAuditText(section.PrintRequest.Request);
            var auditTextSize = e.Graphics.MeasureString(auditText, font7);
            e.Graphics.RotateTransform(-90);
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            Font IFSCFont = new Font("Consolas", 9, FontStyle.Bold);
            ...
            if (BatchInPrinting.ChequeLayout.accountNoVisble)
                e.Graphics.DrawString(string.Format("A/C No.: {0}", section.PrintRequest.Request.AccountNoFull), font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY));

            //if (!string.IsNullOrEmpty(section.PrintRequest.Request.additional_f1))
            //{
            //    if (BatchInPrinting.ChequeLayout.custom1Visible)
            //        e.Graphics.DrawString(string.Format("{0}", section.PrintRequest.Request.additional_f1), font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.custom1X), y + GetDPI(BatchInPrinting.ChequeLayout.custom1Y));
            //}

            if (BatchInPrinting.ChequeLayout.stampVisble)
            {                
                RectangleF stampRect = new RectangleF(x + GetDPI(BatchInPrinting.ChequeLayout.stampX), y + GetDPI(BatchInPrinting.ChequeLayout.stampY), 600, 110);
                // Create a StringFormat object with the each line of text, and the block
                // of text centered on the page.
                StringFormat stringFormat = new StringFormat();
                stringFormat.Alignment = StringAlignment.Far;
