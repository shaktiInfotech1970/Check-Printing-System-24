...
{
    /// <summary>
    /// Interaction logic for TotalReprintedChequeRDLC.xaml
    /// </summary>
    public partial class TotalReprintedChequeRDLC : UserControl
    {
        public TotalReprintedChequeRDLC()
        {
            InitializeComponent();
            BindComboBox();
        }
        ...
        private void RenderReport()
        {
            ...
            _reportviewer.LocalReport.ReportEmbeddedResource = "CPS.Views.Reports.Layout.TotalReprintedCheque.rdlc";
            _reportviewer.LocalReport.EnableExternalImages = true;

            var param = new ReportParameter[2];
            param[0] = new ReportParameter("BankName", string.Format("{0}", BankMasterDTO.GetBankName()));
            param[1] = new ReportParameter("Title", string.Format("Total Re-Printed Cheque:- From:{0}  - To:{1}", (dtPrintDateFrom.SelectedDate ?? DateTime.Now.Date).ToString("dd MMM yyyy"), (dtPrintDateTo.SelectedDate ?? DateTime.Now.Date).ToString("dd MMM yyyy")));
            _reportviewer.LocalReport.SetParameters(param);

            ReportDataSource reportDataSource = new ReportDataSource();
            reportDataSource.Name = "TotalReprintedChequeReportDataSet";
        ...
        private List<TotalPrintReportRDLC> GetData(int branchId, DateTime transactionfromDate, DateTime transactiontoDate)
        {
            using (var context = new CPSDbContext())
            {
                ...
                var query = (from r in repositoryRequest.GetAll()
                             ...
                             && (ph.PrintType == PrintType.RePrintCheque || ph.PrintType == PrintType.RePrintChequeBook)
                ...
                var response = query.Where(o => o.BookSize > 0).ToList();
                if (response.Count == 0)
                {
