...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y, float width, float height)
        {
            ...
            string[] contacts = { request.mob, request.telr, request.telo };
            var filterContacts = contacts.Where(s => !string.IsNullOrWhiteSpace(s)).ToList();
            if (filterContacts != null && filterContacts.Count > 0)
            {
                sb.Append($"{Environment.NewLine}Contact No: {string.Join(", ", filterContacts)}");
            }
        ...
        {
            var sb = new StringBuilder();
            if (IsPayOrder(request.TransactionCode))
            {
                sb.Append("FOR PATAN NAGARIK SAHAKARI BANK LTD.");
                sb.Append(Environment.NewLine + Environment.NewLine + Environment.NewLine + Environment.NewLine);
                sb.Append(Environment.NewLine + Environment.NewLine);
                sb.Append("AUTHORISED SIGNATORIES");
            }
            else
            {
                if (!string.IsNullOrWhiteSpace(request.Name))
        ...
        private static string GetMICR(PrintRequest print, int currentChequeNo, int TransactionCode)
        {
            if (print.Request.AccountNo == "000000")
                return string.Format("C{0}C {1}A {2} {3}", (print.Request.ChequeFrom + (currentChequeNo - 1)).ToString("000000"), print.Branch.MICR, "       ", TransactionCode);
            else
                return string.Format("C{0}C {1}A {2}C {3}", (print.Request.ChequeFrom + (currentChequeNo - 1)).ToString("000000"), print.Branch.MICR, Convert.ToInt32(print.Request.AccountNo.Substring(Math.Max(0, print.Request.AccountNo.Length - 6))).ToString("000000"), TransactionCode);
        }
        ...
        {
            //var lastDate = new DateTime(2021, 9, 29);
        ...
        {
            var section = printPage.Sections.ToList()[sectionNo - 1];
            if (IsAdvancCheque(section.PrintRequest.Request.TransactionCode))
            {
                PrintAdvanceChequeSection(e, printPage, sectionNo);
            }
            else
            {
                SolidBrush brush = new SolidBrush(Color.Black);
                ...
                if (BatchInPrinting.ChequeLayout.orderOrBarerVisble)
                {
                    ...
                    {
                        e.Graphics.DrawString(string.Format("OR {0}", "ORDER"), font9, brush, x + GetDPI(BatchInPrinting.ChequeLayout.orderOrBarerX), y + GetDPI(BatchInPrinting.ChequeLayout.orderOrBarerY));
                    }
                    else
                    {
                ...
                if (BatchInPrinting.ChequeLayout.accountNoVisble)
                {
                    if (IsPayOrder(section.PrintRequest.Request.TransactionCode))
                    {
                        e.Graphics.DrawString(string.Format("BANKER'S CHEQUE {0}", section.PrintRequest.Request.AccountNoFull), font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY));
                    }
                    else
                    {
                ...
                if (BatchInPrinting.ChequeLayout.micrVisble)
                {
                    ...
                    var micrText = GetMICR(section.PrintRequest, section.SequenceNo, section.PrintRequest.Request.TransactionCode);
        ...
        private static void PrintAdvanceChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            SolidBrush brush = new SolidBrush(Color.Black);
            Font font9 = new Font("Verdana", 9);
            Font font7 = new Font("Verdana", 7);
            ...
            if (BatchInPrinting.ChequeLayout.accountNoVisble)
            {
                e.Graphics.DrawString(string.Format("{0}", section.PrintRequest.AccountType.Name), font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY));
            }
            ...
            if (BatchInPrinting.ChequeLayout.micrVisble)
            {
                //e.Graphics.DrawString(GetMICR(section.PrintRequest.Request, section.SequenceNo), micrFont, brush, x + GetDPI(BatchInPrinting.ChequeLayout.micrX), y + GetDPI(BatchInPrinting.ChequeLayout.micrY));
                PointF pointf = new PointF(x + GetDPI(BatchInPrinting.ChequeLayout.micrX), y + GetDPI(BatchInPrinting.ChequeLayout.micrY));
                var micrText = GetMICR(section.PrintRequest, section.SequenceNo, 13);
                var micrX = x + GetDPI(BatchInPrinting.ChequeLayout.micrX);
                var micrY = y + GetDPI(BatchInPrinting.ChequeLayout.micrY);
                foreach (var c in micrText.ToArray())
                {
        ...
        private static bool IsAdvancCheque(int TransactionCode)
        {
