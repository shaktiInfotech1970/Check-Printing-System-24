...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        private static void PrintBlankRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            e.Graphics.DrawString("Regu. Busi. Hrs.: Mon to Fri 10:30 AM to 5:00 PM" + section.PrintRequest.Branch.Time1, font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 1.3f));
            e.Graphics.DrawString("Holidays: Saturday 10:30 AM to 2:00 PM" + section.PrintRequest.Branch.Time2, font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 1.7f));
            e.Graphics.DrawString("Working Holiday: Sunday" + section.PrintRequest.Branch.Time2, font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.1f));
            e.Graphics.DrawString(string.Format("Branch Email : {0}", section.PrintRequest.Branch.Email), font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.5f));
            e.Graphics.DrawString(string.Format("Contact No. : {0}", section.PrintRequest.Branch.Telephone1), font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.9f));
            e.Graphics.DrawString(string.Format("Web Address : {0}", oBankMasterDTO.WebAddress), font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 3.3f));
            /* END Left Section */

            /* START Right Section */

            e.Graphics.DrawString("CHEQUE BOOK REQUEST (BANK COPY)", font8Bold, brush, x + GetDPI(14f), y + GetDPI(0.5f));
            e.Graphics.DrawString("THE MANAGER", font7Bold, brush, x + GetDPI(14f), y + GetDPI(1f));
           ...
           // e.Graphics.DrawString(string.Format("Tel No: {0}", section.PrintRequest.Request.telr), font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 2.5f));
            e.Graphics.DrawString(string.Format("Contact No: {0}", section.PrintRequest.Request.mob), font7, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 3.2f));
            e.Graphics.DrawString("Signature Verified / Authorized", font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X + 7.5f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 1.8f));
            e.Graphics.DrawString("Signature of Bearer", font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 3.8f));

            RectangleF stampRect = new RectangleF(x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X - 3.75f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 0.55f), 450, 110);
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            Font IFSCFont = new Font("Consolas", 11, FontStyle.Bold);
            ...
            if (BatchInPrinting.ChequeLayout.branchAddressVisble)
                e.Graphics.DrawString(GetBranchAddress(section.PrintRequest.Branch), font8Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.branchAddressX), y + GetDPI(BatchInPrinting.ChequeLayout.branchAddressY));
