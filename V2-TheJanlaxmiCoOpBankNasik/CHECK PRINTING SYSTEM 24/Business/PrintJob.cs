...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        private static void GetBranchAddress(PrintPageEventArgs e, BranchMasterDTO branch, int fontsize, float x, float y)
        {
            SolidBrush brush = new SolidBrush(Color.Black);
            Font font = new Font("Verdana", fontsize);
            Font fontBold = new Font("Verdana", fontsize, FontStyle.Bold);


            SizeF stringSize = new SizeF();
            if (!string.IsNullOrWhiteSpace(branch.Name))
            {
                e.Graphics.DrawString(branch.Name, fontBold, brush, x, y);
                stringSize = e.Graphics.MeasureString(string.Format("{0}", branch.Name), font);
            }
            ...
            e.Graphics.DrawString(sb.ToString(), font, brush, x + stringSize.Width, y);
            //return sb.ToString();
        }
        private static string GetBranchAddress1(BranchMasterDTO branch, string bankName)
        {
            ...
            if (!string.IsNullOrWhiteSpace(bankName))
                sb.Append(string.Format("{0} {1}", bankName, branch.City));
            if (!string.IsNullOrWhiteSpace(branch.Name))
            ...
            if (!string.IsNullOrWhiteSpace(branch.AddressLine3))
                sb.Append(", " + branch.AddressLine3);
            if (!string.IsNullOrWhiteSpace(branch.City))
                sb.Append(string.Format("{0}{1} - {2}", ",", branch.City, branch.PostalCode));
            return sb.ToString();
        }
        ...
        {
            var lastDate = new DateTime(2022, 5, 10);
        ...
        private static void PrintBlankRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            e.Graphics.DrawString("For Cheque Above Rupees 5,00,000 PPS", font7, brush, x + GetDPI(0.6f), y + GetDPI(6f));
            e.Graphics.DrawString("Send SMS on 9355492200", font7, brush, x + GetDPI(0.6f), y + GetDPI(6.5f));
            e.Graphics.DrawString("PPS A/C No,Cheque No,Issue Date,Cheque Alpha,Amount,", font7, brush, x + GetDPI(0.6f), y + GetDPI(7f));
            e.Graphics.DrawString("Issuer Name,A/C Type,A/C No (Last 6 Digit)", font7, brush, x + GetDPI(0.6f), y + GetDPI(7.5f));

            e.Graphics.DrawString("BRANCH INFORMATION", font8Bold, brush, x + GetDPI(0.6f + 1f), y + GetDPI(7f + 1.2f));
            e.Graphics.DrawString("Regu. Busi. Hrs.: " + section.PrintRequest.Branch.Time1, font6, brush, x + GetDPI(0.6f), y + GetDPI(7f + 1.8f));
            e.Graphics.DrawString("Half Day Busi Hrs: " + section.PrintRequest.Branch.Time2, font6, brush, x + GetDPI(0.6f), y + GetDPI(7f + 2.2f));
            e.Graphics.DrawString("Weekly Holiday: " + "SUNDAY", font6, brush, x + GetDPI(0.6f), y + GetDPI(7f + 2.6f));
            e.Graphics.DrawString(string.Format("Branch Email-1: {0}", section.PrintRequest.Branch.Email), font6, brush, x + GetDPI(0.6f), y + GetDPI(7f + 3f));
            e.Graphics.DrawString(string.Format("Web Address : {0}", oBankMasterDTO.WebAddress), font6, brush, x + GetDPI(0.6f), y + GetDPI(7f + 3.4f));
            e.Graphics.DrawString(string.Format("Contact No. : {0}", section.PrintRequest.Branch.Telephone1), font6, brush, x + GetDPI(0.6f), y + GetDPI(7f + 3.8f));
            /* END Left Section */
            ...
            e.Graphics.DrawString("Please issue me _________Cheque book(s) containig_________Leaves", font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.branchAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.branchAddress2Y + 1.6f));
            e.Graphics.DrawString("by Register Post/per bearer.", font7Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.branchAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.branchAddress2Y + 2.2f));
            ...
            e.Graphics.DrawString("Signature of Bearer", font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 2.5f));
            e.Graphics.DrawString("Received By", font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 4.5f));
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo, string printChequeBookAuditText)
        {
            ...
            if (BatchInPrinting.ChequeLayout.branchAddressVisble)
                GetBranchAddress(e, section.PrintRequest.Branch, 7, x + GetDPI(BatchInPrinting.ChequeLayout.branchAddressX), y + GetDPI(BatchInPrinting.ChequeLayout.branchAddressY));
