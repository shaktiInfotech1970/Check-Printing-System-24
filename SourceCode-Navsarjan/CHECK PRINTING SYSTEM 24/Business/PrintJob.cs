...
namespace CPS.Business
{
    ...
    public class PrintJob
    {
        ...
        {
            var lastDate = new DateTime(2060, 5, 5);
        ...
        private static void PrintChequeSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            else if (section.PrintRequest.Request.BearerOrder.ToLower() == "bearer" && !string.IsNullOrWhiteSpace(section.PrintRequest.Request.Jointly))
            {
                e.Graphics.DrawString(section.PrintRequest.Request.Jointly.ToUpper(), font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountPayeeX), y + GetDPI(BatchInPrinting.ChequeLayout.accountPayeeY));
            }
