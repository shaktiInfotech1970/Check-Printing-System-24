    ...
    public class PrintJob
    {
        ...
        private static string GetBranchAddress(BranchMasterDTO branch)
        {
            ...
            if (!string.IsNullOrWhiteSpace(branch.AddressLine1))
                sb.Append(branch.AddressLine1 + System.Environment.NewLine);
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y, float width, float height)
        {
            ...
            RectangleF stampRect = new RectangleF(x, y + GetDPI(0.4f), width, height);
            ...
            if (!string.IsNullOrWhiteSpace(request.Address4))
                sb.Append(request.Address3);
            if (!string.IsNullOrWhiteSpace(request.Address5))
                sb.Append(request.Address3);
        ...
        private static void PrintNameAddress(PrintPageEventArgs e, RequestDTO request, float x, float y)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.Name))
                e.Graphics.DrawString(request.Name, fontRegular, brush, x, y);
        ...
        private static string GetStamp(RequestDTO request, string shortName)
        {
            var sb = new StringBuilder();
            if (IsPayOrder(request.TransactionCode, request.BearerOrder))
            {
                sb.Append(string.Format("{0} - {1}", "For PRAGATI SAHAKARI BANK LTD.", shortName));
                sb.Append(Environment.NewLine + Environment.NewLine + Environment.NewLine + Environment.NewLine);
                sb.Append(Environment.NewLine + Environment.NewLine);
                sb.Append("OFFICER/MANAGER");
            }
            else
            {
                ...
                {
                    if (!string.IsNullOrWhiteSpace(request.Name))
                        sb.Append(string.Format("For {0}", request.Name));
        ...
        private static string GetStamp1(RequestDTO request)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.Signatory1) || !string.IsNullOrWhiteSpace(request.Signatory2) || !string.IsNullOrWhiteSpace(request.Signatory3))
            {
                ...
                sb.Append(Environment.NewLine + Environment.NewLine + Environment.NewLine + Environment.NewLine + Environment.NewLine + Environment.NewLine);
            }
            else
            {
                if (!string.IsNullOrWhiteSpace(request.Name))
        ...
        {
            //var lastDate = new DateTime(2022, 2, 15);
        ...
        private static bool SendToPrinter(PrinterPreference printerPreference, PrinterJob printerJob, PrintType printType)
        {
            if (printerPreference != null)
            {
                ...
                {
                    BatchInPrinting = new BatchInPrinting { PageNo = 1, PrintBatch = batch, RequestLayout = requestLayout, ChequeLayout = chequeLayout, PrintType = printType };
        ...
        private static void PrintRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            if (BatchInPrinting.RequestLayout.nameAddress2Visble)
                PrintNameAddress(e, section.PrintRequest.Request, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y), 450, 110);
        ...
        private static void PrintBlankRequestSection(PrintPageEventArgs e, PrintPage printPage, int sectionNo)
        {
            ...
            BankMasterDTO oBankMasterDTO = BranchMasterDTO.GetBankInfo(section.PrintRequest.Branch.MICR.Substring(3, 3));
            ...
            if (BatchInPrinting.RequestLayout.accountNo1Visble)
                e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.AccountNoFull), font8Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.accountNo1X), y + GetDPI(BatchInPrinting.RequestLayout.accountNo1Y));
            ...
            e.Graphics.DrawString("BRANCH INFORMATION", font8Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X + 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 0.8f));
            e.Graphics.DrawString("Regu. Busi. Hrs.: " + section.PrintRequest.Branch.Time1, font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 1.4f));
            e.Graphics.DrawString("Half. Day Busi. Hrs.: " + section.PrintRequest.Branch.Time2, font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 1.8f));
            e.Graphics.DrawString("Sunday Working : " + "Holiday on 2nd & 4th Saturday", font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.2f));
            e.Graphics.DrawString(string.Format("Branch Email-1 : {0}", section.PrintRequest.Branch.Email), font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 2.6f));
            e.Graphics.DrawString(string.Format("Branch Email-2 : {0}", section.PrintRequest.Branch.Email2), font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 3f));
            e.Graphics.DrawString(string.Format("Web Address : {0}", oBankMasterDTO.WebAddress), font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 3.4f));
            e.Graphics.DrawString(string.Format("Contact No.: {0}    {1}", section.PrintRequest.Branch.Telephone1, section.PrintRequest.Branch.Telephone2), font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 3.8f));
            e.Graphics.DrawString(string.Format("Toll Free No.:"), font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 4.2f));
            e.Graphics.DrawString(string.Format("Branch Service:"), font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1X - 0.6f), y + GetDPI(BatchInPrinting.RequestLayout.chequeFrom1Y + 4.6f));
            /* END Left Section */

            /* START Right Section */

            e.Graphics.DrawString("CHEQUE BOOK REQUEST (BANK COPY)", font8Bold, brush, x + GetDPI(12.8f), y + GetDPI(0.5f));
            e.Graphics.DrawString("THE MANAGER", font7Bold, brush, x + GetDPI(11f), y + GetDPI(1.1f));
            ...
            e.Graphics.DrawString("Date: ____________________", font7Bold, brush, x + GetDPI(21f), y + GetDPI(1.1f));
            ...
            e.Graphics.DrawString("Signature Verified / Authorized", font6Bold, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X + 9.9f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 0.8f));
            e.Graphics.DrawString("Signature of Bearer", font6, brush, x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 4.5f));

            RectangleF stampRect = new RectangleF(x + GetDPI(BatchInPrinting.RequestLayout.nameAddress2X - 1.2f), y + GetDPI(BatchInPrinting.RequestLayout.nameAddress2Y + 0.8f), 450, 110);
            ...
            // Draw the text and the surrounding rectangle.
            e.Graphics.DrawString(GetStamp1(section.PrintRequest.Request), font8, brush, stampRect, stringFormat);
        ...
        private static void chequePrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            if (printPage != null)
            {
                ...
                if (BatchInPrinting.PrintType == PrintType.RePrintCheque || BatchInPrinting.PrintType == PrintType.RePrintChequeBook)
                {
        ...
        {
            SolidBrush brush = new SolidBrush(Color.Black);
            Font font10 = new Font("Verdana", 10);
            Font font9 = new Font("Verdana", 9);
            Font font7 = new Font("Verdana", 7);
            Font font6 = new Font("Verdana", 6);
            Font font8 = new Font("Verdana", 8);
            ...
            if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder))
            {
                RectangleF stampRect = new RectangleF(x, y + GetDPI(0.8f), 525, 10);
                StringFormat stringFormat = new StringFormat();
                stringFormat.Alignment = StringAlignment.Far;
                stringFormat.LineAlignment = StringAlignment.Center;
                e.Graphics.DrawString("PAY ORDER", font9Bold, brush, stampRect, stringFormat);
            }
            ...
            if (BatchInPrinting.ChequeLayout.accountNoVisble)
            {
                if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder))
                {
                    e.Graphics.DrawString(string.Format("{0}", section.PrintRequest.Request.AccountNoFull), font10, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY));
                }
                else
                {
                    e.Graphics.DrawString(string.Format("{0}: {1}", section.PrintRequest.AccountType.Name, section.PrintRequest.Request.AccountNoFull), font10, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY));
                }
            }
            if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder))
            {
                e.Graphics.DrawString("PAYORDER", font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX + 10f), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY + 1f));
            }

            if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder))
            {
                e.Graphics.DrawString("Not Over Rs.:", font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX + 5.5f), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY + 1.6f));
            }
            if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder))
            {
                e.Graphics.DrawString("On Account Of ___________________________", font9Bold, brush, x + GetDPI(BatchInPrinting.ChequeLayout.accountNoX), y + GetDPI(BatchInPrinting.ChequeLayout.accountNoY + 3.2f));
            }
            if (BatchInPrinting.ChequeLayout.custom1Visible && (!string.IsNullOrEmpty(section.PrintRequest.Request.additional_f1)))
                e.Graphics.DrawString(string.Format("{0}", section.PrintRequest.Request.additional_f1, section.PrintRequest.Request.AccountNoFull), font9, brush, x + GetDPI(BatchInPrinting.ChequeLayout.custom1X), y + GetDPI(BatchInPrinting.ChequeLayout.custom1Y));
            ...
            if (BatchInPrinting.ChequeLayout.stampVisble)
            {
                ...
                //if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder))
                //{
                //    stampRect = new RectangleF(x + GetDPI(BatchInPrinting.ChequeLayout.stampX + 5f), y + GetDPI(BatchInPrinting.ChequeLayout.stampY), 450, 110);                    
                //}
                // Draw the text and the surrounding rectangle.

                e.Graphics.DrawString(GetStamp(section.PrintRequest.Request, section.PrintRequest.Branch.ShortName), font8, brush, stampRect, stringFormat);
            }
            ...
            if (IsPayOrder(section.PrintRequest.Request.TransactionCode, section.PrintRequest.Request.BearerOrder) && BatchInPrinting.ChequeLayout.accountPayeeVisble)
            {
        ...
        private static bool IsPayOrder(int TransactionCode, string BearerOrder)
        {
            return (TransactionCode == 12 && BearerOrder.ToLower() == "order");
        }
        #endregion
